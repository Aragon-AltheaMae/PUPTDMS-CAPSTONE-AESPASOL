  ...
  <main id="mainContent"
    ...
    <div style="max-width:1280px;margin:0 auto;">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        @php
        $statCards = [
        ['icon'=>'fa-calendar-days','color'=>'from-[#8B0000] to-[#6B0000]','val'=>$openDays, 'label'=>'Open Days/Week',
        'sub'=>'Active schedule days'],
        ['icon'=>'fa-clock', 'color'=>'from-blue-500 to-blue-600', 'val'=>$maxSlots, 'label'=>'Daily Slot Cap',
        'sub'=>'Max patients/day'],
        ['icon'=>'fa-ban', 'color'=>'from-green-500 to-green-600','val'=>$blockedThisMonth,'label'=>'Blocked
        Dates','sub'=>'This month'],
        ['icon'=>'fa-umbrella-beach','color'=>'from-amber-500
        to-amber-600','val'=>$holidaysThisMonth,'label'=>'Holidays','sub'=>'This month'],
        ];
        @endphp
            ...
            <div class="p-4 overflow-x-auto">
              ...
              <div class="flex flex-wrap gap-3 mt-3 justify-end">
                <div class="flex items-center gap-1.5 text-xs text-gray-500"><span
                    class="w-3 h-3 rounded bg-blue-200 border-l-2 border-blue-500 inline-block"></span>Check-up</div>
                <div class="flex items-center gap-1.5 text-xs text-gray-500"><span
                    class="w-3 h-3 rounded bg-green-200 border-l-2 border-green-500 inline-block"></span>Cleaning</div>
                <div class="flex items-center gap-1.5 text-xs text-gray-500"><span
                    class="w-3 h-3 rounded bg-yellow-100 border-l-2 border-yellow-400 inline-block"></span>Surgery</div>
                <div class="flex items-center gap-1.5 text-xs text-gray-500"><span
                ...
                <tbody>
                  ...
                  <tr>
                    <td colspan="7" class="text-center py-10">
                      ...
                      <p class="text-gray-400 text-sm">No rules yet. <button onclick="openRuleModal()"
                          class="text-[#8B0000] font-semibold hover:underline">Add one.</button></p>
                    </td>
                  </tr>
                  @endforelse
                </tbody>
            ...
            <div class="p-4 space-y-0.5">
              ...
              <div class="pt-2 mt-1 border-t border-gray-100">
                <div class="flex justify-between items-center">
                  ...
                  <span class="text-xs font-medium text-gray-500">{{ date('g:i A',strtotime(trim($bs).':00')) }} – {{
          ...
          <div class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden">
            ...
            <div class="p-4">
              ...
              <div class="flex items-start gap-3 py-2.5 {{ !$loop->last ? 'border-b border-gray-50':'' }}">
                <div
                  class="w-9 h-9 rounded-lg bg-red-50 border border-red-100 flex items-center justify-center flex-shrink-0 text-[#8B0000] text-xs font-bold">
                  {{ $bd->day }}</div>
                <div class="flex-1 min-w-0">
                  ...
                  @if($blocked->note)<p class="text-[10px] text-gray-400 mt-0.5 italic truncate">{{ $blocked->note }}
                  </p>@endif
                </div>
                <form action="{{ route('admin.clinic_schedule.unblock',$blocked) }}" method="POST">
                  ...
                  <button type="submit"
                    ...
                    title="Remove"><i class="fa-solid fa-xmark text-xs"></i></button>
                </form>
              </div>
              @empty
              <div class="text-center py-6"><i class="fa-solid fa-check-circle text-3xl text-green-400 mb-2 block"></i>
            ...
            <div class="p-4">
              ...
              <div class="flex items-center gap-3 py-2 border-b border-gray-50 last:border-b-0">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-[10px] text-gray-400">{{ $diff===0?'Today':($diff===1?'Tomorrow':"In $diff days") }}
                  </p>
                </div>
      ...
      <div class="modal-hdr">
        <div class="flex items-center justify-between">
          ...
          <button onclick="closeRuleModal()"
            class="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white hover:bg-white/30 transition-colors"><i
      ...
      <div class="modal-body">
        <form id="ruleForm" method="POST" action="{{ route('admin.clinic_schedule.store') }}">
          ...
          <div id="ruleTimeFields">
            ...
            <div class="mb-4">
              ...
              <div class="flex gap-2 flex-wrap">
                <div class="break-chip selected" data-val="12:00-13:00" onclick="selectBreak(this)">12:00 – 1:00 PM
                </div>
                <div class="break-chip" data-val="13:00-14:00" onclick="selectBreak(this)">1:00 – 2:00 PM</div>
                <div class="break-chip" data-val="none" onclick="selectBreak(this)"><i
                    class="fa-solid fa-ban text-[10px]"></i> No Break</div>
              </div>
            </div>
            ...
            <div class="mb-4">
              ...
              <div class="flex items-center gap-3">
                <button type="button" onclick="adjSlots(-1)"
                  class="w-9 h-9 rounded-lg border border-gray-200 flex items-center justify-center text-gray-600 hover:border-[#8B0000] hover:text-[#8B0000] font-bold text-lg">−</button>
                <input type="number" id="ruleMaxSlots" class="form-ctrl text-center font-bold text-lg w-20" value="5"
                  min="1" max="30">
                <button type="button" onclick="adjSlots(1)"
                  class="w-9 h-9 rounded-lg border border-gray-200 flex items-center justify-center text-gray-600 hover:border-[#8B0000] hover:text-[#8B0000] font-bold text-lg">+</button>
              </div>
          ...
          <div class="mb-5">
            ...
            <textarea id="ruleNotes" class="form-ctrl resize-none" rows="2"
              placeholder="e.g. Holiday schedule, reduced hours…"></textarea>
          </div>

          <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
            <button type="button" onclick="closeRuleModal()"
              class="px-5 py-2 rounded-xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50">Cancel</button>
            <button type="button" onclick="submitRule()"
              class="px-5 py-2 rounded-xl bg-[#8B0000] hover:bg-[#760000] text-white text-sm font-bold shadow"><i
                class="fa-solid fa-floppy-disk mr-1.5"></i>Save Rule</button>
          </div>
        </form>
      </div>
    </div>
  ...
  <div id="blockModalBackdrop" class="modal-backdrop" onclick="closeBlockModal()">
    <div class="modal-box" style="width:440px;" onclick="event.stopPropagation()">
      <div class="modal-hdr">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="text-lg font-bold">Block Date</h3>
            <p class="text-sm text-white/70 mt-0.5">Mark a date as unavailable for bookings</p>
          </div>
      ...
      <div class="modal-body">
        <form action="{{ route('admin.clinic_schedule.block') }}" method="POST">
          ...
          <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
            ...
            <button type="submit"
              class="px-5 py-2 rounded-xl bg-[#8B0000] hover:bg-[#760000] text-white text-sm font-bold shadow"><i
                class="fa-solid fa-ban mr-1.5"></i>Block Date</button>
          </div>
        </form>
      </div>
    </div>
