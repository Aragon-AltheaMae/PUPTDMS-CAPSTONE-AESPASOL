    ...
    <style>
        ...
        *::after {
        ...
        .header {
            ...
            justify-content: space-between; box-shadow: 0 2px 20px rgba(139,0,0,.25);
        }
        ...
        .notif-btn {
            ...
            color: #fff; font-size: .95rem; display: flex; align-items: center;
            justify-content: center; transition: background .15s; position: relative;
        }
        .notif-btn:hover { background: rgba(255,255,255,.22); }
        .notif-badge {
            position: absolute; top: -3px; right: -3px; background: #ff6b6b; color: #fff;
            font-size: .6rem; font-weight: 700; width: 16px; height: 16px; border-radius: 50%;
            display: flex; align-items: center; justify-content: center; border: 2px solid #8B0000;
        }
        ...
        .flyout-title {
            ...
            color: #8B0000; padding: 4px 8px 6px; border-bottom: 1px solid #fde8e8; margin-bottom: 4px;
        }
        .flyout-link {
            display: flex; align-items: center; gap: 8px; padding: 7px 10px; border-radius: 8px;
            font-size: .77rem; font-weight: 500; color: #374151; text-decoration: none; transition: all .15s; white-space: nowrap;
        }
        ...
        .theme-toggle-container {
            ...
            width: 100%; height: 34px; background: #F5F5F5; border: 1px solid #E0E0E0;
        ...
        .theme-option {
            position: relative; z-index: 2; flex: 1; height: 40px; display: flex;
            align-items: center; justify-content: center; background: transparent;
            border: none; cursor: pointer; color: #9CA3AF; transition: color .2s ease; border-radius: 8px;
        }
        #sidebar.collapsed .theme-option { width: 35px; height: 40px; flex: none; }
        .theme-option i { font-size: 16px; }
        #sidebar.collapsed .theme-option i { font-size: 15px; }
        .theme-option.active { color: #374151; }
        .theme-indicator {
            position: absolute; background: white; border-radius: 24px;
            box-shadow: 0 2px 8px rgba(0,0,0,.1); transition: all .3s cubic-bezier(.4,0,.2,1); pointer-events: none;
        }
        ...
        .logout-btn {
            ...
            color: #ef4444; font-size: .77rem; font-weight: 600; transition: background .15s;
        }
        ...
        [data-theme="dark"] .group-header:hover, [data-theme="dark"] .nav-link:hover { background: rgba(139,0,0,.2); }
        ...
        [data-theme="dark"] .sidebar-brand-text { color: #f87171; }

        /* ── Role cards ── */
        .role-card {
            background: #FDFCFB; border: 1.5px solid #EDE8E2; border-radius: 14px;
            padding: 14px 16px; cursor: pointer; transition: all 0.2s cubic-bezier(.4,0,.2,1);
        }
        .role-card:hover { transform: translateX(3px); }
        .role-card.active { background: #fff; border-color: #7B0D0D; box-shadow: 0 4px 20px rgba(123,13,13,0.12); }
        .role-avatar {
            width: 42px; height: 42px; border-radius: 12px; display: flex;
            align-items: center; justify-content: center; font-weight: 800; font-size: 13px;
            flex-shrink: 0; transition: all 0.2s; background: #F0EBE6; color: #8A7A6F;
        }
        .role-card.active .role-avatar { background: linear-gradient(135deg,#7B0D0D,#9B1515); color: #fff; box-shadow: 0 4px 10px rgba(123,13,13,0.3); }
        .badge-pill { display: inline-block; padding: 2px 10px; border-radius: 20px; font-size: 11px; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; }
        ...
        .group-card {
            background: #fff; border: 1.5px solid #EDE8E2; border-radius: 14px;
            overflow: hidden; box-shadow: 0 1px 4px rgba(0,0,0,0.04);
            transition: opacity 0.4s ease, transform 0.4s ease;
        }
        .perm-group-header {
            display: flex; align-items: center; padding: 14px 20px;
            background: #FDFCFB; cursor: pointer; transition: background 0.15s; user-select: none;
        }
        .perm-group-header:hover { background: #FAF4EF; }
        .perm-group-icon { width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 15px; flex-shrink: 0; margin-right: 14px; }
        ...
        .all-toggle-wrap { display: flex; align-items: center; gap: 8px; background: #F5EFE9; border-radius: 8px; padding: 5px 12px; cursor: pointer; }
        .toggle-switch { position: relative; width: 46px; height: 26px; display: inline-block; flex-shrink: 0; }
        .toggle-switch input { opacity: 0; width: 0; height: 0; position: absolute; }
        .toggle-track {
            position: absolute; cursor: pointer; inset: 0; background: #F9FAFB;
            border: 2px solid #E5E7EB; border-radius: 13px; transition: all 0.25s cubic-bezier(.4,0,.2,1);
        }
        .toggle-track::after {
            content: ''; position: absolute; top: 3px; left: 3px; width: 16px; height: 16px;
            border-radius: 50%; background: #D1D5DB; box-shadow: 0 1px 4px rgba(0,0,0,0.25);
            transition: left 0.25s cubic-bezier(.4,0,.2,1), background 0.2s;
        }
        .toggle-switch input:checked + .toggle-track { background: linear-gradient(135deg,#7B0D0D,#9B1515); border-color: #7B0D0D; box-shadow: 0 0 0 3px rgba(123,13,13,0.13); }
        .toggle-switch input:checked + .toggle-track::after { left: 22px; background: #fff; }
        .toggle-switch.disabled .toggle-track { cursor: not-allowed; opacity: 0.45; }
        .perm-row { display: flex; align-items: center; padding: 12px 20px 12px 70px; border-bottom: 1px solid #F5F0EB; transition: background 0.15s; }
        ...
        .perm-group-body {
            border-top: 1px solid #F0EBE6; overflow: hidden;
            transition: max-height 0.35s ease, opacity 0.25s ease; max-height: 9999px; opacity: 1;
        }
        ...
        .footer-bar {
            ...
            padding: 18px 24px; background: #fff; border-radius: 14px;
            border: 1.5px solid #EDE8E2; margin-top: 18px; gap: 12px; flex-wrap: wrap;
        }
        .btn-reset {
            background: #F5EFE9; color: #6B5E56; border: none; border-radius: 10px;
            padding: 10px 20px; font-weight: 600; font-size: 14px; cursor: pointer;
            font-family: 'Inter', sans-serif; transition: background 0.15s;
        }
        .btn-reset:hover { background: #EDE5DA; }
        .btn-save {
            background: linear-gradient(135deg,#7B0D0D,#9B1515); color: #fff; border: none;
            border-radius: 10px; padding: 10px 28px; font-weight: 700; font-size: 14px; cursor: pointer;
            font-family: 'Inter', sans-serif; box-shadow: 0 4px 14px rgba(123,13,13,0.25);
            transition: all 0.2s; display: flex; align-items: center; gap: 8px;
        }
        .btn-save:hover { transform: translateY(-1px); box-shadow: 0 8px 24px rgba(123,13,13,0.35); }
        .search-input {
            width: 100%; padding: 10px 14px 10px 38px; border: 1.5px solid #EDE8E2; border-radius: 10px;
            font-size: 14px; font-family: 'Inter', sans-serif; background: #fff; outline: none;
            color: #2D2420; transition: border-color 0.18s;
        }
        .search-input:focus { border-color: #7B0D0D; }
        .btn-collapse {
            padding: 10px 16px; border: 1.5px solid #EDE8E2; border-radius: 10px; background: #fff;
            font-size: 13px; font-family: 'Inter', sans-serif; cursor: pointer; color: #6B5E56;
        ...
        .accent-card { background: linear-gradient(135deg,#7B0D0D 0%,#9B1515 100%); border-radius: 14px; padding: 18px 20px; color: #fff; margin-top: 16px; }
        @keyframes fadeUp { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }
        .fade-up { animation: fadeUp 0.45s ease both; }
        .modal-overlay { position: fixed; inset: 0; background: rgba(15,5,5,0.55); display: flex; align-items: center; justify-content: center; z-index: 200; backdrop-filter: blur(4px); }
        .modal-box { background: #fff; border-radius: 20px; padding: 36px 36px 28px; width: 420px; box-shadow: 0 32px 80px rgba(0,0,0,0.25); }
        ...
        .btn-view-as {
            ...
            box-shadow: 0 4px 14px rgba(29,78,216,0.3); transition: all 0.2s;
            position: relative; white-space: nowrap;
        }
        .btn-view-as.show { display: flex; animation: popSlide .45s cubic-bezier(.34,1.56,.64,1) both; }
        .btn-view-as:hover { transform: translateY(-1px); box-shadow: 0 8px 24px rgba(29,78,216,.4); }
        @keyframes popSlide { from { opacity:0; transform:translateX(16px) scale(.9); } to { opacity:1; transform:translateX(0) scale(1); } }
        .va-count-badge {
        ...
        .hint-save {
            display: none; align-items: center; gap: 6px; background: #EFF6FF;
            border: 1px solid #BFDBFE; border-radius: 8px; padding: 6px 12px;
            font-size: 12px; font-weight: 600; color: #1D4ED8;
        }
        .hint-save.show { display: flex; }

        /* NEW: View As modal */
        .va-overlay {
            position: fixed; inset: 0; background: rgba(15,5,5,0.6); display: flex;
        ...
        .va-panel {
            background: #fff; border-radius: 22px; width: 680px;
            max-width: calc(100vw - 32px); max-height: calc(100vh - 60px);
            overflow: hidden; display: flex; flex-direction: column;
            box-shadow: 0 40px 100px rgba(0,0,0,.3);
            transform: scale(.94) translateY(16px);
            transition: transform .35s cubic-bezier(.34,1.56,.64,1);
        }
        .va-overlay.open .va-panel { transform: scale(1) translateY(0); }
        .va-head { padding: 22px 26px 18px; border-bottom: 1px solid #F0EBE6; display: flex; align-items: flex-start; gap: 14px; }
        .va-body { flex: 1; overflow-y: auto; padding: 20px 26px; }
        .va-foot { padding: 14px 26px 20px; border-top: 1px solid #F0EBE6; display: flex; justify-content: flex-end; }
        .va-summary {
            background: linear-gradient(135deg,#7B0D0D,#9B1515); border-radius: 12px;
            padding: 14px 18px; display: flex; align-items: center; gap: 14px; margin-bottom: 18px; color: #fff;
        }
        .va-role-row {
            display: flex; align-items: center; gap: 14px; background: #FDFCFB;
            border: 1.5px solid #EDE8E2; border-radius: 14px; padding: 16px 18px;
            cursor: pointer; transition: all .2s; position: relative; overflow: hidden; margin-bottom: 10px;
        }
        .va-role-row:last-child { margin-bottom: 0; }
        .va-role-row:hover { border-color: #93C5FD; box-shadow: 0 4px 18px rgba(29,78,216,.1); transform: translateY(-1px); }
        .va-role-avatar { width: 44px; height: 44px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 14px; flex-shrink: 0; }
        .va-perm-tag {
        ...
        .va-go-btn {
            ...
            flex-shrink: 0; transition: all .2s; box-shadow: 0 3px 10px rgba(29,78,216,.25);
        }
        .va-go-btn:hover { transform: translateY(-1px); box-shadow: 0 6px 18px rgba(29,78,216,.35); }
        ...
        /* NEW: Toast */
        .toast-pop {
            position: fixed; bottom: 28px; right: 28px; z-index: 600; background: #fff;
            border: 1.5px solid #BBF7D0; border-radius: 14px; padding: 14px 20px;
            display: flex; align-items: center; gap: 12px; box-shadow: 0 8px 32px rgba(0,0,0,0.12);
            transform: translateY(80px); opacity: 0;
            transition: all .4s cubic-bezier(.34,1.56,.64,1); min-width: 300px;
        }
        .toast-pop.show { transform: translateY(0); opacity: 1; }
        .toast-icon { width: 36px; height: 36px; border-radius: 10px; background: #F0FDF4; display: flex; align-items: center; justify-content: center; font-size: 16px; color: #22C55E; flex-shrink: 0; }
        ...
        .card-new { animation: cardSlide 0.4s cubic-bezier(.34,1.56,.64,1) both; }
    </style>
</head>
...
<body>
        ...
        <div class="max-w-7xl mt-4 mx-auto fade-in">
            ...
            <div style="display:grid; grid-template-columns:280px 1fr; gap:24px; align-items:start;">
                ...
                <div>
                    ...
                    <div style="font-size:11px; color:#B5A99A; letter-spacing:2px; text-transform:uppercase; margin-bottom:12px; font-weight:600;" id="roleCountLabel">
                        Roles ({{ $roles->count() }})
                    </div>
                        ...
                        <div class="role-card {{ $isFirst ? 'active' : '' }}"
                            ...
                            <div style="margin-top:12px;">
                                ...
                                <div class="progress-bar">
                                    <div class="progress-fill" style="width:{{ $pct }}%; background:{{ $isFirst ? 'linear-gradient(90deg,#7B0D0D,#C9973A)' : '#C4B8AF' }};"></div>
                                </div>
                    ...
                    <div class="accent-card">
                        ...
                        @endphp
                        <div style="font-size:16px; font-weight:700; margin-bottom:4px;" id="accentRoleName">{{ $fr->name ?? '' }}</div>
                        <div style="font-size:28px; font-weight:700; margin-bottom:2px;" id="accentPct">{{ $fp }}%</div>
                        <div style="font-size:12px; opacity:0.75; margin-bottom:14px;" id="accentCount">{{ $fr?->permissions->count() ?? 0 }} of {{ $totalPerms }} permissions active</div>
                ...
                <div>
                    ...
                    <div style="display:flex; align-items:center; gap:12px; margin-bottom:18px; flex-wrap:wrap;">
                        <div style="flex:1; position:relative; min-width:180px;">
                            ...
                            <input type="text" id="permSearch" placeholder="Search permissions…" class="search-input" oninput="filterPerms(this.value)">
                        </div>
                        ...
                        <div style="display:flex; flex-direction:column; gap:10px;" class="groups-container">
                            ...
                            <div class="group-card perm-group" data-group="{{ strtolower($module) }}">
                                ...
                                <div class="perm-group-header" onclick="togglePermGroup(this)">
                                    <div class="perm-group-icon" style="background:{{ $icol }}15; color:{{ $icol }}; border:1px solid {{ $icol }}25;">
    ...
    <div class="va-overlay" id="vaOverlay" onclick="if(event.target===this)closeViewAs()">
        <div class="va-panel">
            <div class="va-head">
                <div style="width:46px;height:46px;border-radius:13px;background:linear-gradient(135deg,#EFF6FF,#DBEAFE);border:1.5px solid #BFDBFE;display:flex;align-items:center;justify-content:center;font-size:19px;color:#1D4ED8;flex-shrink:0;">
                    <i class="fa-solid fa-eye"></i>
                </div>
                <div style="flex:1;">
                    <div style="font-size:19px;font-weight:800;color:#1E293B;margin-bottom:3px;">View As — Select Role</div>
                    <div style="font-size:13px;color:#8A7A6F;" id="vaSubtitle">Select a role to preview their dashboard access</div>
                </div>
                <button onclick="closeViewAs()" style="margin-left:auto;width:34px;height:34px;border-radius:9px;background:#F5EFE9;border:none;cursor:pointer;color:#8A7A6F;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0;">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
            <div class="va-body">
                <div class="va-summary">
                    <div style="width:38px;height:38px;border-radius:10px;background:rgba(255,255,255,.15);display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0;">
                        <i class="fa-solid fa-shield-halved"></i>
                    </div>
                    <div>
                        <div style="font-size:12px;opacity:.75;margin-bottom:2px;">Total newly granted &amp; saved permissions</div>
                ...
                <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#B5A99A;margin-bottom:10px;">Roles with newly granted access</div>
                <div id="vaRoleList"></div>
            </div>
            <div class="va-foot">
                <button onclick="closeViewAs()" style="background:#F5EFE9;color:#6B5E56;border:none;border-radius:10px;padding:10px 22px;font-weight:600;font-size:14px;cursor:pointer;font-family:'Inter',sans-serif;">
                    Close
                </button>
            </div>
        </div>
    ...
    <div id="newRoleModal" style="display:none;position:fixed;inset:0;background:rgba(15,5,5,0.55);align-items:center;justify-content:center;z-index:200;backdrop-filter:blur(4px);">
        <div style="background:#fff;border-radius:20px;padding:36px 36px 28px;width:440px;box-shadow:0 32px 80px rgba(0,0,0,0.25);">
            ...
            <div id="newRoleError" style="display:none;color:#B91C1C;font-size:13px;margin-bottom:12px;background:#FEF2F2;border-radius:8px;padding:8px 12px;"></div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.role-form').forEach(form => {
        ...
        const modules = [...new Set(Array.from(toggles).map(t => t.dataset.module).filter(Boolean))];
...
function openViewAs() {
    ...
    Object.entries(savedGrants).forEach(([roleId, perms]) => {
        ...
        list.innerHTML += `
            <div class="va-role-row" onclick="redirectToRole('${roleId}','${roleName}','${color}')">
                ...
                <button class="va-go-btn" onclick="event.stopPropagation();redirectToRole('${roleId}','${roleName}','${color}')">
    ...
    document.getElementById('vaSubtitle').textContent = `${totalRoles} role${totalRoles > 1 ? 's' : ''} with newly granted access — select one to redirect`;
...
function createNewRole() {
    ...
    card.onclick = function () { selectRole(this); };
    ...
    document.getElementById('roleCountLabel').textContent = `Roles (${document.querySelectorAll('.role-card').length})`;
...
function getRoleBadgeJS(name, slug) {
    ...
    if (n.includes('super') || s.includes('super')) return { color:'#7B0D0D', label:'Full Access' };
    if (n.includes('dentist') || s.includes('dentist')) return { color:'#B45309', label:'Clinical' };
    if (n.includes('staff') || s.includes('staff') || n.includes('clinic')) return { color:'#065F46', label:'Front Desk' };
    if (n.includes('student') || s.includes('student') || n.includes('patient')) return { color:'#4B5563', label:'Limited' };

    return { color:'#6B7280', label:'Custom' };
}
...
function buildRoleForm(roleId, roleName, slug, accentColor) {
    ...
    form.innerHTML = `<div style="display:flex;flex-direction:column;gap:10px;" class="groups-container">${groupsHtml}</div>${footerHtml}`;
