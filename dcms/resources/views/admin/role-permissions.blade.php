...
<style>
    /* ══════════════════════════════════════
       TOAST NOTIFICATION STYLES (OVERRIDES DAISYUI)
    ══════════════════════════════════════ */
    #toastContainer {
    ...
    #toastContainer .toast.error::before { background: #DC2626 !important; }
    #toastContainer .toast.success::before { background: #16a34a !important; }
    ...
    .toast.error .toast-icon-wrap { background: #FEF2F2 !important; }
    .toast.success .toast-icon-wrap { background: #F0FDF4 !important; }

    .toast-icon { font-size: 15px !important; margin: 0 !important; }
    .toast.error .toast-icon { color: #DC2626 !important; }
    .toast.success .toast-icon { color: #16a34a !important; }
    ...
    .toast-close:hover { color: #6b7280 !important; background: transparent !important; }

    /* Dark mode overrides for Toasts */
    [data-theme="dark"] #toastContainer > div { background: #161b22 !important; box-shadow: 0 10px 40px rgba(0, 0, 0, .4) !important; }
    [data-theme="dark"] .toast-title { color: #f3f4f6 !important; }
    [data-theme="dark"] .toast.error .toast-icon-wrap { background: rgba(239, 68, 68, .1) !important; }
    [data-theme="dark"] .toast.success .toast-icon-wrap { background: rgba(34, 197, 94, .1) !important; }

    /* ══════════════════════════════════════
       SHARED DASHBOARD STYLES & HEADER
    ══════════════════════════════════════ */
    * { box-sizing: border-box; }
    ...
    ══════════════════════════════════════ */
    .role-list-header {
        font-size: .65rem; color: #6b7280; letter-spacing: .08em;
        text-transform: uppercase; margin-bottom: 12px; font-weight: 800;
    }

    .role-card {
        background: #fff; border: 1px solid rgba(0,0,0,.06); border-radius: 14px;
        padding: 14px 16px; cursor: pointer; transition: all 0.2s cubic-bezier(.4, 0, .2, 1);
        margin-bottom: 10px; position: relative; box-shadow: 0 2px 8px rgba(0,0,0,.02);
    }
    .role-card:last-child { margin-bottom: 0; }
    .role-card:hover { transform: translateX(3px); box-shadow: 0 4px 12px rgba(0,0,0,.05); }
    ...
    .role-card.active .role-avatar {
        ...
        color: #fff; box-shadow: 0 4px 10px rgba(139, 0, 0, 0.25);
    }

    .badge-pill {
        display: inline-block; padding: 2px 10px; border-radius: 20px;
        font-size: .65rem; font-weight: 700; letter-spacing: 0.02em; text-transform: uppercase;
    }

    .progress-bar { height: 4px; background: #f3f4f6; border-radius: 10px; overflow: hidden; margin: 4px 0; }
    .progress-fill { height: 100%; border-radius: 10px; transition: width 0.4s ease; background: #d1d5db; }
    .role-card.active .progress-fill { background: linear-gradient(90deg, var(--crimson), #e11d48); }

    .btn-delete-role {
        position: absolute; top: 10px; right: 10px; width: 26px; height: 26px;
        border-radius: 7px; border: none; background: transparent; color: #9ca3af;
        font-size: 11px; cursor: pointer; display: flex; align-items: center; justify-content: center;
        opacity: 0; transition: all .15s; z-index: 10;
    }
    .role-card:hover .btn-delete-role { opacity: 1; }
    .btn-delete-role:hover { background: #FEE2E2; color: #DC2626; }
    ...
    ══════════════════════════════════════ */
    .card { background: #fff; border-radius: 16px; border: 1px solid rgba(0, 0, 0, .05); box-shadow: 0 4px 20px rgba(0, 0, 0, .03); overflow: hidden; }
    .card-header { padding: 1rem 1.25rem; border-bottom: 1px solid #f3f4f6; display: flex; align-items: center; justify-content: space-between; background: #fafafa; flex-wrap: wrap; gap: 10px;}
    
    .st-input-wrap { position: relative; }
    .st-input-icon { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: #9ca3af; font-size: 12px; pointer-events: none; }
    .st-input {
        width: 100%; padding: 10px 14px 10px 34px; border: 1px solid #e5e7eb; border-radius: 10px;
        font-size: .8rem; font-family: 'Inter', sans-serif; outline: none; transition: all .2s;
    }
    .st-input:focus { border-color: var(--crimson); box-shadow: 0 0 0 3px rgba(139, 0, 0, .1); }

    .btn-collapse, .btn-reset {
        padding: 9px 14px; border: 1px solid #e5e7eb; border-radius: 10px; background: #fff;
        font-size: .75rem; font-weight: 600; cursor: pointer; color: #4b5563; transition: all .15s; font-family: 'Inter', sans-serif;
    }
    .btn-collapse:hover { background: #f9fafb; border-color: #d1d5db; }
    .btn-reset:hover { background: #fef2f2; border-color: #fca5a5; color: var(--crimson); }
    ...
    /* Permission Groups */
    .group-card {
        background: #fff; border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden; margin-bottom: 10px;
    }
    .perm-group-header {
        display: flex; align-items: center; padding: 12px 16px; background: #fafafa; cursor: pointer; transition: background 0.15s; user-select: none;
    }
    .perm-group-header:hover { background: #f3f4f6; }
    
    .perm-group-icon {
        width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center;
        justify-content: center; font-size: 13px; flex-shrink: 0; margin-right: 12px;
    }
    ...
    .all-toggle-wrap { display: flex; align-items: center; gap: 8px; background: #fff; border: 1px solid #e5e7eb; border-radius: 8px; padding: 4px 10px; cursor: pointer; }

    /* Custom Toggle Switch */
    .toggle-switch { position: relative; width: 42px; height: 24px; display: inline-block; flex-shrink: 0; margin: 0; }
    .toggle-switch input { opacity: 0; width: 0; height: 0; position: absolute; }
    .toggle-track {
        position: absolute; cursor: pointer; inset: 0; background: #e5e7eb; border-radius: 12px; transition: all 0.2s;
    }
    .toggle-track::after {
        content: ''; position: absolute; top: 2px; left: 2px; width: 20px; height: 20px;
        border-radius: 50%; background: #fff; box-shadow: 0 1px 3px rgba(0,0,0,.2); transition: all 0.2s;
    }
    .toggle-switch input:checked + .toggle-track { background: var(--crimson); }
    .toggle-switch input:checked + .toggle-track::after { transform: translateX(18px); }
    .toggle-switch.disabled .toggle-track { cursor: not-allowed; opacity: 0.5; }

    .perm-group-body { border-top: 1px solid #f3f4f6; overflow: hidden; transition: max-height 0.3s ease, opacity 0.2s ease; max-height: 9999px; opacity: 1; }
    .perm-group-body.collapsed { max-height: 0; opacity: 0; border-top: none; }
    .chevron { transition: transform 0.2s; color: #9ca3af; font-size: 11px; }
    .chevron.collapsed { transform: rotate(180deg); }

    .perm-row { display: flex; align-items: center; padding: 12px 16px 12px 60px; border-bottom: 1px solid #f9fafb; transition: background 0.15s; }
    .perm-row:last-child { border-bottom: none; }
    .perm-row:hover { background: #fdfcfb; }

    .status-granted { font-size: 10px; font-weight: 700; padding: 3px 8px; border-radius: 20px; text-transform: uppercase;}
    .status-denied { font-size: 10px; font-weight: 700; padding: 3px 8px; border-radius: 20px; background: #f3f4f6; color: #9ca3af; text-transform: uppercase;}

    /* View As Toolbar Button */
    .btn-view-as {
        display: none; align-items: center; gap: 8px; background: #f0f9ff; color: #1d4ed8; border: 1px solid #bfdbfe;
        border-radius: 10px; padding: 9px 14px; font-weight: 600; font-size: .75rem; cursor: pointer; transition: all 0.15s; position: relative; white-space: nowrap; font-family: 'Inter', sans-serif;
    }
    .btn-view-as.show { display: flex; }
    .btn-view-as:hover { background: #e0f2fe; border-color: #93c5fd; }
    ...
    .va-count-badge {
        position: absolute; top: -6px; right: -6px; background: #ef4444; color: #fff; font-size: 9px;
    ...
    ══════════════════════════════════════ */
    .modern-modal {
        border: none; padding: 32px 24px 24px; border-radius: 20px; width: min(90vw, 400px);
        box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); background: #ffffff; text-align: center; overflow: visible;
    }
    .modern-modal::backdrop { background: rgba(17, 24, 39, 0.6); backdrop-filter: blur(4px); }
    
    .modal-icon {
        width: 64px; height: 64px; border-radius: 50%; display: flex; align-items: center; justify-content: center;
        margin: 0 auto 16px; font-size: 26px; border: 6px solid #fff;
    }
    .modal-icon.danger { background: #fef2f2; color: #ef4444; box-shadow: 0 0 0 1px #fee2e2; }
    .modal-icon.warning { background: #fffbeb; color: #f59e0b; box-shadow: 0 0 0 1px #fef3c7; }
    .modal-icon.primary { background: #eff6ff; color: #3b82f6; box-shadow: 0 0 0 1px #dbeafe; }
    .modal-icon.success { background: #f0fdf4; color: #22c55e; box-shadow: 0 0 0 1px #bbf7d0; }

    .modal-title { font-size: 1.15rem; font-weight: 800; color: #111827; margin: 0 0 8px; }
    .modal-body { font-size: .85rem; color: #6b7280; line-height: 1.6; margin: 0 0 24px; }
    
    .modal-highlight {
        display: inline-block; font-size: .9rem; font-weight: 700; color: #111827; background: #f3f4f6;
        padding: 4px 10px; border-radius: 6px; margin: 8px 0; word-break: break-all;
    }

    .modal-actions { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
    .modal-btn-cancel {
        padding: 10px; border-radius: 10px; border: 1px solid #e5e7eb; background: #ffffff; color: #374151;
        font-weight: 600; font-size: .85rem; cursor: pointer; transition: all .15s;
    }
    .modal-btn-cancel:hover { background: #f9fafb; border-color: #d1d5db; }
    
    .modal-btn-confirm {
        padding: 10px; border-radius: 10px; border: none; color: #ffffff !important; font-weight: 600; font-size: .85rem;
        cursor: pointer; transition: all .15s; width: 100%;
    }
    .modal-btn-confirm.danger { background: #ef4444; }
    .modal-btn-confirm.danger:hover { background: #dc2626; box-shadow: 0 4px 12px rgba(220, 38, 38, 0.2); }
    .modal-btn-confirm.warning { background: #f59e0b; }
    .modal-btn-confirm.warning:hover { background: #d97706; box-shadow: 0 4px 12px rgba(245, 158, 11, 0.2); }
    .modal-btn-confirm.primary { background: linear-gradient(135deg, var(--crimson), var(--crimson-dark)); }
    .modal-btn-confirm.primary:hover { transform: translateY(-1px); box-shadow: 0 4px 14px rgba(139, 0, 0, 0.3); }

    .modal-form-group { text-align: left; margin-bottom: 16px; }
    .modal-label { display: block; font-size: .7rem; font-weight: 700; color: #4b5563; margin-bottom: 6px; text-transform: uppercase; letter-spacing: .05em; }
    
    /* View As Panel (Specific Modal) */
    #vaOverlay, #patientPickerOverlay {
        position: fixed; inset: 0; background: rgba(17, 24, 39, 0.6); display: flex; align-items: center; justify-content: center;
        z-index: 300; backdrop-filter: blur(4px); opacity: 0; pointer-events: none; transition: opacity .25s;
    }
    #vaOverlay.open, #patientPickerOverlay.open { opacity: 1; pointer-events: auto; }
    .va-panel {
        background: #fff; border-radius: 20px; width: 640px; max-width: 94vw; max-height: 85vh; overflow: hidden; display: flex; flex-direction: column;
        box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); transform: scale(.95); transition: transform .3s;
    }
    #vaOverlay.open .va-panel, #patientPickerOverlay.open .va-panel { transform: scale(1); }
    
    .va-head { padding: 20px 24px; border-bottom: 1px solid #f3f4f6; display: flex; align-items: center; gap: 14px; background: #fafafa; }
    .va-body { flex: 1; overflow-y: auto; padding: 20px 24px; }
    .va-foot { padding: 16px 24px; border-top: 1px solid #f3f4f6; display: flex; justify-content: flex-end; background: #fafafa;}
    
    .va-role-row {
        display: flex; align-items: center; gap: 14px; background: #fff; border: 1px solid #e5e7eb; border-radius: 12px;
        padding: 14px 16px; cursor: pointer; transition: all .2s; margin-bottom: 10px;
    }
    .va-role-row:hover { border-color: #93c5fd; box-shadow: 0 4px 12px rgba(59, 130, 246, .1); transform: translateY(-1px); }
    .va-go-btn {
        display: flex; align-items: center; gap: 6px; background: #eff6ff; color: #1d4ed8; border: none; border-radius: 8px;
        padding: 8px 14px; font-weight: 700; font-size: .75rem; cursor: pointer; transition: all .2s;
    }
    .va-go-btn:hover { background: #dbeafe; }

    /* Redirect Overlay */
    .redirect-overlay { position: fixed; inset: 0; z-index: 9999; display: flex; flex-direction: column; align-items: center; justify-content: center; opacity: 0; pointer-events: none; transition: opacity .3s; }
    .redirect-overlay.show { opacity: 1; pointer-events: auto; }
    .redirect-spinner { width: 48px; height: 48px; border: 3px solid rgba(255, 255, 255, .2); border-top-color: #fff; border-radius: 50%; animation: spin .7s linear infinite; margin-bottom: 16px; }
    @keyframes spin { to { transform: rotate(360deg); } }

    /* Dark Mode Overrides */
    [data-theme="dark"] .card { background: #161b22; border-color: #21262d; }
    [data-theme="dark"] .card-header, [data-theme="dark"] .perm-group-header, [data-theme="dark"] .va-head, [data-theme="dark"] .va-foot { background: #0d1117; border-color: #21262d; }
    [data-theme="dark"] .st-input { background: #0d1117; border-color: #21262d; color: #f3f4f6; }
    [data-theme="dark"] .role-card, [data-theme="dark"] .group-card, [data-theme="dark"] .va-role-row { background: #161b22; border-color: #21262d; }
    [data-theme="dark"] .role-card:hover { background: #1c2128; }
    [data-theme="dark"] .perm-row { border-color: #1c2128; }
    [data-theme="dark"] .perm-row:hover { background: #1c2128; }
    [data-theme="dark"] .modern-modal, [data-theme="dark"] .va-panel { background: #1f2937; }
    [data-theme="dark"] .page-title, [data-theme="dark"] .modal-title, [data-theme="dark"] .role-name-label { color: #f9fafb; }
    [data-theme="dark"] .page-subtitle, [data-theme="dark"] .modal-body, [data-theme="dark"] .role-list-header { color: #9ca3af; }
    [data-theme="dark"] .modal-btn-cancel, [data-theme="dark"] .btn-collapse, [data-theme="dark"] .btn-reset { background: #374151; border-color: #4b5563; color: #d1d5db; }
    [data-theme="dark"] .modal-highlight { background: #374151; color: #f3f4f6; }
    [data-theme="dark"] .floating-save-bar { background: #1f2937; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.5); border: 1px solid #374151; }
    [data-theme="dark"] .btn-save-float { background: #fff; color: #111827; }
    [data-theme="dark"] .fsb-sub { color: #9ca3af; }
    [data-theme="dark"] .btn-view-as.fsb-view-as { background: rgba(59, 130, 246, 0.1); color: #60a5fa; border-color: rgba(59, 130, 246, 0.2); }

    @media (max-width: 1024px) { .main-grid { grid-template-columns: 1fr; } }
    @media (max-width: 767px) {
        .page-header { flex-direction: column; align-items: stretch; }
        .btn-new-role { justify-content: center; }
        .role-list-container { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; }
        .role-card { margin-bottom: 0; }
        .card-header { flex-direction: column; align-items: stretch; }
        .btn-collapse, .btn-reset { flex: 1; text-align: center; justify-content: center; display: flex;}
    }
</style>
@endsection
        ...
        <div class="main-grid">
            ...
            <div>
                ...
                function getRoleBadge($name, $slug) {
                    ...
                    if (str_contains($n,'dentist') || str_contains($s,'dentist')) return ['badgeColor'=>'#d97706','label'=>'Clinical'];
                    if (str_contains($n,'staff') || str_contains($s,'staff') || str_contains($n,'clinic')) return ['badgeColor'=>'#059669','label'=>'Front Desk'];
                ...
                <div class="role-list-container">
                    ...
                    @php
                        ...
                        $isSuperRole = in_array(strtolower($role->slug),['super_admin','super-admin','superadmin']) || str_contains(strtolower($role->name),'super');
                    ...
                    <div class="role-card {{ $isFirst ? 'active' : '' }}" data-role-id="{{ $role->id }}"
                        ...
                        <button type="button" class="btn-delete-role"
                            onclick="event.stopPropagation(); openDeleteModal('{{ $role->id }}', '{{ addslashes($role->name) }}')" title="Delete role">
                        ...
                        <div style="display:flex; align-items:center; gap:12px;">
                            ...
                            <div style="flex:1; min-width:0;">
                                <div style="display:flex; align-items:center; gap:7px; margin-bottom:3px;">
                                    <span style="font-weight:700; font-size:13px; color:#1f2937; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" class="role-name-label">{{ $role->name }}</span>
                                </div>
                                <div style="display:flex; align-items:center; gap:6px;">
                                    <span class="badge-pill" style="background:{{ $c['badgeColor'] }}15; color:{{ $c['badgeColor'] }};">{{ $c['label'] }}</span>
                                </div>
                            </div>
                        </div>

                        <div style="margin-top:12px;">
                            <div style="display:flex; justify-content:space-between; font-size:10px; color:#9ca3af; font-weight:700; text-transform:uppercase;">
                                <span>Access</span>
                ...
                <div class="accent-card">
                    @php
                        $fr = isset($highlightRoleId) ? $roles->firstWhere('id',(int)$highlightRoleId) : $roles->first();
                        $fp = $fr ? ($totalPerms > 0 ? round(($fr->permissions->count()/$totalPerms)*100) : 0) : 0;
                    @endphp
                    <div style="font-size:15px; font-weight:800; margin-bottom:4px;" id="accentRoleName">{{ $fr->name ?? '' }}</div>
                    <div style="font-size:32px; font-weight:900; margin-bottom:2px; line-height:1;" id="accentPct">{{ $fp }}%</div>
                    <div style="font-size:11px; opacity:0.8; margin-bottom:14px;" id="accentCount">{{ $fr?->permissions->count() ?? 0 }} of {{ $totalPerms }} active</div>
                    <div style="height:4px; background:rgba(255,255,255,0.2); border-radius:10px;">
                        <div id="accentBar" style="height:100%; width:{{ $fp }}%; background:#fff; border-radius:10px; transition:width 0.4s;"></div>
                    </div>
                </div>
            </div>
                ...
                <div class="card" style="display:flex; flex-direction:column; min-height:600px;">
                    <div class="card-header">
                        <div class="st-input-wrap" style="flex:1; min-width:240px;">
                            ...
                            <input type="text" id="permSearch" placeholder="Search permissions..." class="st-input" oninput="filterPerms(this.value)">
                        </div>
                    ...
                    <div style="padding: 1.25rem 1.25rem 0; flex:1;">
                        <div class="protected-banner" id="protectedBanner" style="display:none;">
                            ...
                            <div>
                                ...
                                <div style="font-size:12px; color:#b45309;">Super Admin has unrestricted access and cannot be modified.</div>
                            </div>
                        </div>
                        ...
                        <form id="form-role-{{ $role->id }}" class="role-form" data-role-id="{{ $role->id }}" style="display:{{ $isActiveRole ? 'block' : 'none' }}; height: 100%;">
                            @csrf
                            <input type="hidden" name="role_id" value="{{ $role->id }}">

                            <div style="display:flex; flex-direction:column; gap:10px; padding-bottom: 80px;" class="groups-container">
                                ...
                                @php
                                    ...
                                    $roleGranted = 0;
                                    foreach ($permissions as $_p) { if ($role->permissions->contains('id',$_p->id)) $roleGranted++; }
                                ...
                                <div class="group-card perm-group" data-group="{{ strtolower($module) }}">
                                    <div class="perm-group-header" onclick="togglePermGroup(this)">
                                        <div class="perm-group-icon" style="background:{{ $icol }}15; color:{{ $icol }};">
                                            <i class="fa-solid {{ $ico }}"></i>
                                        </div>
                                        <div style="flex:1;">
                                            <div style="font-weight:800; font-size:13px; color:#1f2937;">{{ $module }}</div>
                                            <div style="font-size:11px; color:#6b7280; font-weight: 600;" class="group-count">{{ $roleGranted }} of {{ $mTotal }} enabled</div>
                                        </div>
                                        <div style="display:flex; align-items:center; gap:14px;">
                                            <div class="dot-row" id="dots-{{ $role->id }}-{{ $mSlug }}">
                                                @for ($d = 0; $d < $mTotal; $d++) 
                                                    <div class="dot" style="{{ $d < $roleGranted ? 'background:'.$icol.';' : '' }}"></div>
                                                @endfor
                                            </div>
                                            <div class="all-toggle-wrap" onclick="event.stopPropagation(); toggleGroupPerms(this,'{{ $role->id }}','{{ $mSlug }}',{{ $allOn ? 'true' : 'false' }})">
                                                <span style="font-size:10px; color:#4b5563; font-weight:700; text-transform:uppercase;">All</span>
                                                <label class="toggle-switch {{ $isSuperRole ? 'disabled' : '' }}" onclick="event.preventDefault();">
                                                    <input type="checkbox" class="group-master" data-role="{{ $role->id }}" data-module="{{ $mSlug }}" {{ $allOn ? 'checked' : '' }} {{ $isSuperRole ? 'disabled' : '' }}>
                                    ...
                                    <div class="perm-group-body">
                                        ...
                                        <div class="perm-row" data-perm-search="{{ strtolower($permission->name.' '.$permission->slug) }}">
                                            <div style="flex:1;">
                                                <div style="display:flex; align-items:center; gap:8px; margin-bottom:2px;">
                                                    <span style="font-weight:700; font-size:12px; color:{{ $isGranted ? '#1f2937' : '#6b7280' }};" class="perm-label">{{ $permission->name }}</span>
                                                </div>
                                                <div style="font-size:11px; color:#9ca3af;">{{ $permission->slug }}</div>
                                            </div>
                                            <div style="display:flex; align-items:center; gap:10px;">
                                                <span class="perm-status {{ $isGranted ? 'status-granted' : 'status-denied' }}" style="{{ $isGranted ? 'background:'.$icol.'18; color:'.$icol.';' : '' }}">
                                                    {{ $isGranted ? 'Granted' : 'Denied' }}
                                                </span>
                                                <label class="toggle-switch {{ $isSuperRole ? 'disabled' : '' }}">
                                                    <input type="checkbox" name="permissions[{{ $role->id }}][]" value="{{ $permission->id }}" class="perm-toggle"
                                                        data-role="{{ $role->id }}" data-module="{{ $mSlug }}" data-color="{{ $icol }}" data-perm-name="{{ $permission->name }}"
                                                        data-perm-slug="{{ $permission->slug }}" {{ $isGranted ? 'checked' : '' }} {{ $isSuperRole ? 'disabled' : '' }} onchange="onPermChange(this)">
                                                    <span class="toggle-track"></span>
                                                </label>
                                            </div>
                                        </div>
                                ...
                                <div style="text-align:center; padding:60px 20px;">
                                    <i class="fa-solid fa-shield-halved" style="font-size:40px; color:#e5e7eb; margin-bottom:12px;"></i>
                                    <p style="font-size:14px; font-weight:700; color:#6b7280;">No permissions found.</p>
                                </div>
                                @endforelse
                            </div>
                            ...
                            <div class="floating-save-bar" id="footer-bar-{{ $role->id }}">
                                ...
                                <div class="fsb-actions">
                                    ...
                                    <button type="button" class="btn-discard" onclick="discardChanges('{{ $role->id }}')">Discard</button>
                                    <button type="button" class="btn-save-float" id="save-btn-{{ $role->id }}" onclick="ajaxSaveRole('{{ $role->id }}')">
    ...
    <form id="createRoleForm" action="{{ route('admin.role_permissions.store_role') }}" method="POST">
        ...
        <div class="modal-form-group">
            ...
            <div class="st-input-wrap">
                ...
                <input type="text" id="newRoleName" name="name" class="st-input" placeholder="e.g. Dental Intern" autocomplete="off">
            </div>
        </div>
        <div class="modal-form-group">
            ...
            <div class="st-input-wrap">
                ...
                <input type="text" id="newRoleSlug" name="slug" class="st-input" placeholder="e.g. dental-intern" autocomplete="off">
            </div>
        </div>
        
        <div id="newRoleError" style="display:none; color:#dc2626; font-size:12px; font-weight:600; margin-bottom:16px; background:#fef2f2; padding:8px; border-radius:8px;"></div>
...
<dialog id="deleteRoleModal" class="modern-modal">
    ...
    <div class="modal-body">
        ...
        <span style="display:block; color:#ef4444; font-weight:600; margin-top:8px;">This action cannot be undone.</span>
    </div>
    <div class="modal-actions">
...
<dialog id="resetConfirmModal" class="modern-modal">
    ...
    <div class="modal-body">
        This restores original permissions for <strong style="color:#111827;">Super Admin</strong>, <strong style="color:#111827;">Dentist</strong>, and <strong style="color:#111827;">Patient</strong>.
        <div style="background:#fffbeb; border:1px solid #fde68a; border-radius:8px; padding:10px; margin-top:12px; color:#b45309; font-size:.8rem; font-weight:600; text-align:left;">
            <i class="fa-solid fa-triangle-exclamation" style="margin-right:4px;"></i> Custom changes will be lost. This cannot be undone.
        </div>
    </div>
    <div class="modal-actions">
        <button type="button" class="modal-btn-cancel" onclick="closeResetConfirm()">Cancel</button>
        <button type="button" id="resetConfirmBtn" class="modal-btn-confirm warning" onclick="confirmResetDefaults()">Yes, Reset</button>
    </div>
</dialog>
...
<div id="vaOverlay">
    <div class="va-panel">
        <div class="va-head">
            <div style="width:40px;height:40px;border-radius:10px;background:#eff6ff;color:#3b82f6;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;">
                <i class="fa-solid fa-eye"></i>
            </div>
            <div style="flex:1;">
                <div style="font-size:1.1rem;font-weight:800;color:#111827;margin-bottom:2px;">View As Role</div>
                <div style="font-size:.8rem;color:#6b7280;" id="vaSubtitle">Select a role to preview dashboard access</div>
            </div>
            <button onclick="closeViewAs()" style="width:32px;height:32px;border-radius:8px;background:#f3f4f6;border:none;cursor:pointer;color:#6b7280;"><i class="fa-solid fa-xmark"></i></button>
        </div>
        <div class="va-body">
            <div style="background: linear-gradient(135deg, var(--crimson), var(--crimson-dark)); border-radius: 12px; padding: 14px 18px; display: flex; align-items: center; gap: 14px; margin-bottom: 16px; color: #fff;">
                <i class="fa-solid fa-shield-halved" style="font-size: 20px; opacity: .8;"></i>
                <div>
                    <div style="font-size:11px; opacity:.8; margin-bottom:2px; text-transform:uppercase; font-weight:700;">Newly granted & saved</div>
                    <div><strong style="font-size:20px;" id="vaTotalPerms">0</strong> permissions across <strong style="font-size:20px;" id="vaTotalRoles">0</strong> roles</div>
                </div>
            </div>
...
<div id="patientPickerOverlay">
    <div class="va-panel">
        <div class="va-head">
            <div style="width:40px;height:40px;border-radius:10px;background:#f0fdf4;color:#22c55e;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;">
                <i class="fa-solid fa-user-injured"></i>
            </div>
            <div style="flex:1;">
                <div style="font-size:1.1rem;font-weight:800;color:#111827;margin-bottom:2px;">Select Patient Account</div>
                <div style="font-size:.8rem;color:#6b7280;">Choose which patient to impersonate</div>
            </div>
            <button onclick="closePatientPicker()" style="width:32px;height:32px;border-radius:8px;background:#f3f4f6;border:none;cursor:pointer;color:#6b7280;"><i class="fa-solid fa-xmark"></i></button>
        </div>
        <div class="va-body">
            <div class="st-input-wrap" style="margin-bottom: 16px;">
                <i class="fa-solid fa-magnifying-glass st-input-icon"></i>
                <input type="text" id="patientPickerSearch" placeholder="Search patient name or email..." class="st-input" oninput="filterPatientPicker(this.value)">
            </div>
...
<script>
    // PERM MODULE META
    const PERM_MODULES = [
        { module: 'Dashboard', color: '#8B0000' },
        { module: 'Patients', color: '#d97706' },
        { module: 'Appointments', color: '#059669' },
        { module: 'Document Requests', color: '#2563eb' },
        { module: 'Document Template', color: '#7c3aed' },
        { module: 'Reports', color: '#7c3aed' },
        { module: 'Academic Periods', color: '#059669' },
        { module: 'Data Backup', color: '#ea580c' },
        { module: 'System Logs', color: '#dc2626' },
        { module: 'System Settings', color: '#4b5563' },
    ];
    ...
    function initRoleForms() {
        ...
        document.querySelectorAll('.role-form').forEach(form => {
            ...
            form.querySelectorAll('.perm-toggle').forEach(input => {
                ...
                if (input.checked) {
                    savedGrants[roleId].push({ name: input.dataset.permName || '', slug: input.dataset.permSlug || '', color: input.dataset.color || '#4b5563' });
                }
            });
    ...
    function updateDots(roleId, mSlug, color) {
        ...
        toggles.forEach((t, i) => { if (dots[i]) dots[i].style.background = t.checked ? color : '#e5e7eb'; });
    }
    ...
    document.getElementById('createRoleForm').addEventListener('submit', function(e) {
        ...
        if (!name || !slug) { errEl.textContent = 'Please fill out all fields.'; errEl.style.display = 'block'; return; }
        ...
        .then(() => {
            ...
            .then(html => {
                ...
                if (newGrid) {
                    ...
                    // Automatically click/highlight the newly created role
                    const newRoleCard = document.querySelector(`.role-card[data-slug="${slug}"]`) || document.querySelector('.role-card');
    ...
    function confirmResetDefaults() {
        ...
        const csrf = document.querySelector('meta[name="csrf-token"]')?.content || '{{ csrf_token() }}';
        confirmBtn.disabled = true; confirmBtn.innerHTML = 'Resetting…';

        fetch('{{ route("admin.role_permissions.reset") }}', {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' }, body: JSON.stringify({})
        })
        .then(async res => {
            const data = await res.json().catch(() => ({}));
            if (!res.ok) throw new Error(data.message || 'Server error ' + res.status);
    ...
    function openViewAs() {
        ...
        document.querySelectorAll('.role-card').forEach(card => {
            ...
            const checkedPerms = [...form.querySelectorAll('.perm-toggle:checked')].map(input => ({
                name: input.dataset.permName || 'Permission', color: input.dataset.color || '#4b5563'
            }));
            ...
            const tags = checkedPerms.map(p => `<span style="font-size:10px; font-weight:700; color:${p.color}; background:${p.color}15; padding:2px 8px; border-radius:12px;">${p.name}</span>`).join('');
            const goBtn = !isSuperAdmin ? `<button class="va-go-btn va-redirect-btn" data-role-id="${roleId}" data-role-name="${roleName}" data-role-slug="${roleSlug}" data-color="${color}">Go to Dashboard <i class="fa-solid fa-arrow-right"></i></button>` : '';
        ...
        list.querySelectorAll('.va-redirect-btn').forEach(el => {
            el.addEventListener('click', function (e) {
                e.stopPropagation(); const t = this.closest('[data-role-id]') || this;
    ...
    function redirectToRole(roleId, roleName, roleSlug, color) {
        ...
        closeViewAs(); triggerRedirect(roleName, roleSlug, null, color, `Loading ${roleName} view for Super Admin`);
    }
    ...
    function renderPatientPicker(patients) {
        ...
        if (!list) return;
        if (!patients.length) { list.innerHTML = '<div style="text-align:center;padding:32px 20px;color:#6b7280;font-size:14px;">No patients found.</div>'; return; }
        list.innerHTML = patients.map(p => {
            const n = (p.name || 'Patient').replace(/'/g, "\\'"); const i = (p.name || 'P').charAt(0).toUpperCase();
    ...
    function startPatientImpersonation(roleName, roleSlug, color, patientId, patientName) {
        closePatientPicker(); triggerRedirect(patientName, roleSlug, patientId, color, 'Loading patient dashboard for Super Admin');
    }
