...
<head>
  ...
  <style>
    ...
    .header-logo-box { width:36px; height:36px; border-radius:8px; background:rgba(255,255,255,.18); display:flex; align-items:center; justify-content:center; }
    ...
    .group-icon { width:34px; height:34px; border-radius:8px; flex-shrink:0; display:flex; align-items:center; justify-content:center; font-size:15px; transition:all .2s; }
    ...
    @media (min-width:1024px) {
      .active-banner-inner { flex-direction:row; align-items:center; justify-content:space-between; gap:1.25rem; }
    }

    .progress-track { height:6px; border-radius:99px; background:#f3f4f6; overflow:hidden; }
    .progress-fill  { height:100%; border-radius:99px; background:linear-gradient(90deg,#8B0000,#c0392b); transition:width .6s cubic-bezier(.4,0,.2,1); }

    .sem-pill { display:inline-flex; align-items:center; gap:5px; font-size:11px; font-weight:700; padding:3px 10px; border-radius:99px; }

    .s-active   { background:#fee2e2; color:#8B0000; }
    .s-upcoming { background:#dbeafe; color:#1d4ed8; }
    .s-ended    { background:#f3f4f6; color:#6b7280; }
    .s-inactive { background:#fef9c3; color:#92400e; }

    .act { padding:6px 9px; border-radius:8px; border:none; cursor:pointer; font-size:12px; transition:all .15s; display:inline-flex; align-items:center; gap:4px; font-weight:600; }
    .act:hover { transform:scale(1.06); }
    .act-edit   { background:#eff6ff; color:#2563eb; }
    .act-edit:hover   { background:#dbeafe; }
    .act-star   { background:#d1fae5; color:#065f46; }
    .act-star:hover   { background:#a7f3d0; }
    .act-del    { background:#fef2f2; color:#dc2626; }
    .act-del:hover    { background:#fee2e2; }
    .act-pinned { background:#d1fae5; color:#065f46; opacity:.55; cursor:default; }

    .tbl-row { transition:background .12s; }
    .tbl-row:hover { background:#fef9f9; }
    .tbl-row.is-active { background:#fff7f7; }
    ...
    [data-theme="dark"] .modal-box textarea { background:#0d1117 !important; border-color:#21262d !important; color:#e5e7eb !important; }
    [data-theme="dark"] .tbl-row:hover { background:#0d1117; }
    [data-theme="dark"] .tbl-row.is-active { background:rgba(139,0,0,.07); }
    [data-theme="dark"] thead tr { background:#0d1117 !important; }
    [data-theme="dark"] tr { border-color:#21262d !important; }
    ...
    [data-theme="dark"] .active-banner { background:#161b22 !important; }
    [data-theme="dark"] .progress-track { background:#21262d; }
    [data-theme="dark"] .modal-box .bg-gray-50 { background:#0d1117 !important; }
    [data-theme="dark"] .cal-card { background:#161b22 !important; border-color:#21262d !important; }
  </style>
</head>
...
<header class="header">
  <div class="header-left">
    <div class="header-logo-box"><i class="fa-solid fa-tooth text-white text-base"></i></div>
  ...
  <div class="header-right">
    <div id="notifDropdown">
      ...
      <div id="notifMenu">
        ...
        <div>
          <div style="padding:.65rem 1rem;font-size:.78rem;border-bottom:1px solid #fdf5f5;">
            <div style="font-weight:600;">Period Ending Soon</div>
            <div style="color:#aaa;margin-top:2px;">Monitor your active academic period regularly.</div>
          </div>
          <div style="padding:.65rem 1rem;font-size:.78rem;">
            <div style="font-weight:600;">Academic Period Management</div>
            <div style="color:#aaa;margin-top:2px;">You can add, update, activate, and delete academic periods here.</div>
          </div>
        </div>
      </div>
    </div>
    <div class="header-user">
      <img src="https://i.pravatar.cc/40?img=5" class="header-avatar" alt="Admin">
      <div>
        <div class="header-name">Admin</div>
        <div class="header-role">Super Admin</div>
      </div>
    </div>
  </div>
</header>
  ...
  <div class="sidebar-inner">
    <div class="nav-group flyout-wrapper" id="group-cms">
      <div class="group-header active-group" onclick="toggleGroup('cms',event)">
        <div class="group-icon" style="background:#8B0000;color:#fff;box-shadow:0 4px 12px rgba(139,0,0,.3);">
        ...
        <i class="fa-solid fa-chevron-down group-chevron open" id="chevron-cms"></i>
      </div>
      <div class="group-body open" id="body-cms">
        <a href="#" class="nav-link"><i class="fa-solid fa-chart-line"></i> Dashboard</a>
        ...
        <a href="#" class="nav-link active"><i class="fa-solid fa-school"></i> Academic Periods</a>
        <a href="#" class="nav-link"><i class="fa-solid fa-file-circle-check"></i> Document Requests</a>
        <a href="#" class="nav-link"><i class="fa-solid fa-file-pen"></i> Document Template</a>
        <a href="#" class="nav-link"><i class="fa-solid fa-file"></i> Reports</a>
      </div>
      <div class="flyout-panel" id="flyout-cms">
        <div class="flyout-title">Clinic Management</div>
        <a href="#" class="flyout-link"><i class="fa-solid fa-chart-line"></i> Dashboard</a>
        <a href="#" class="flyout-link"><i class="fa-solid fa-users"></i> Patients</a>
        <a href="#" class="flyout-link"><i class="fa-solid fa-calendar-check"></i> Appointments</a>
        <a href="#" class="flyout-link"><i class="fa-solid fa-school"></i> Academic Periods</a>
        <a href="#" class="flyout-link"><i class="fa-solid fa-file-circle-check"></i> Document Requests</a>
        <a href="#" class="flyout-link"><i class="fa-solid fa-file-pen"></i> Document Template</a>
        <a href="#" class="flyout-link"><i class="fa-solid fa-file"></i> Reports</a>
      </div>
    </div>
    ...
    <div class="nav-group flyout-wrapper" id="group-sys">
      ...
      <div class="group-body open" id="body-sys">
        <a href="#" class="nav-link"><i class="fa-solid fa-database"></i> Data Backup</a>
        <a href="#" class="nav-link"><i class="fa-solid fa-clipboard-list"></i> System Logs</a>
        <a href="#" class="nav-link"><i class="fa-solid fa-gear"></i> System Settings</a>
        <a href="#" class="nav-link"><i class="fa-solid fa-user-shield"></i> Roles &amp; Permissions</a>
      </div>
      <div class="flyout-panel" id="flyout-sys">
        <div class="flyout-title">System</div>
        <a href="#" class="flyout-link"><i class="fa-solid fa-database"></i> Data Backup</a>
        <a href="#" class="flyout-link"><i class="fa-solid fa-clipboard-list"></i> System Logs</a>
        <a href="#" class="flyout-link"><i class="fa-solid fa-gear"></i> System Settings</a>
        <a href="#" class="flyout-link"><i class="fa-solid fa-user-shield"></i> Roles &amp; Permissions</a>
      </div>
    </div>
  </div>
      ...
      <div class="active-banner-inner">
        <div class="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-5">
          <div>
            ...
            <p class="text-xl font-bold text-gray-800" id="bannerSem">{{ $activePeriod?->semester ?? 'No Active Period' }}</p>
          </div>
          <div>
            <div class="flex items-center gap-2 mb-1">
          ...
          <div>
            ...
            <p class="text-xl font-bold text-gray-800" id="bannerEnd">{{ $activePeriod ? $activePeriod->end_date->format('F d, Y') : '—' }}</p>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-3 lg:flex-shrink-0 lg:w-64">
          <div>
            <div class="flex justify-between items-center mb-1.5">
              ...
              <span class="text-[11px] font-bold text-[#8B0000]" id="bannerPct">{{ $activePeriod?->progress_percent ?? 0 }}%</span>
            </div>
            ...
            <p class="text-[10px] text-gray-400 mt-1" id="bannerDaysLeft">
              {{ $activePeriod ? $activePeriod->days_remaining . ' day' . ($activePeriod->days_remaining !== 1 ? 's' : '') . ' remaining' : 'No active period' }}
            </p>
          </div>
      ...
      <div class="lg:col-span-2">
        <div class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden">
          ...
          <div class="px-5 py-4 border-b bg-gray-50 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
            ...
            <form method="GET" action="{{ route('admin.academic_periods') }}" class="flex flex-wrap items-center gap-2">
              <div class="relative">
                <i class="fa-solid fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" style="font-size:11px;"></i>
                <input
                  type="text"
                  name="search"
              ...
              <a href="{{ route('admin.academic_periods') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-xs font-semibold text-gray-600">
                Reset
              </a>
            </form>
          </div>
                  ...
                  <tr class="tbl-row {{ $period->is_active ? 'is-active' : '' }} border-b border-gray-50 last:border-0">
                    ...
                    <td class="py-3 px-4">
                      <div class="flex items-center">
                        @if($period->is_active)
                          <span class="dot-pulse" style="display:inline-block;width:7px;height:7px;border-radius:50%;background:#22c55e;margin-right:6px;"></span>
                        @else
                          <span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:#e5e7eb;margin-right:6px;"></span>
                        @endif
                    ...
                    <td class="py-3 px-4 text-xs text-gray-600">{{ optional($period->start_date)->format('M d, Y') }}</td>
                    <td class="py-3 px-4 text-xs text-gray-600">{{ optional($period->end_date)->format('M d, Y') }}</td>

                    <td class="py-3 px-4 text-center">
                      <span class="status-badge {{ $statusClass }}" style="display:inline-flex;align-items:center;gap:3px;font-size:11px;font-weight:700;padding:3px 9px;border-radius:99px;">
                        {{ $period->status }}
                      </span>
                    </td>
          ...
          <div class="px-5 py-3.5 border-t border-gray-100 bg-gray-50 flex items-center justify-between gap-3">
            <p class="text-xs text-gray-500">
              Showing {{ $academicPeriods->firstItem() ?? 0 }}–{{ $academicPeriods->lastItem() ?? 0 }} of {{ $academicPeriods->total() }} periods
            </p>
            ...
            <div class="flex items-center gap-1.5">
              ...
              @else
                <a href="{{ $academicPeriods->previousPageUrl() }}" class="page-btn">
              ...
              @for($page = 1; $page <= $academicPeriods->lastPage(); $page++)
                ...
                @else
                  <a href="{{ $academicPeriods->url($page) }}" class="page-btn">{{ $page }}</a>
                @endif
              @endfor

              @if($academicPeriods->hasMorePages())
                <a href="{{ $academicPeriods->nextPageUrl() }}" class="page-btn">
      ...
      <div class="space-y-5">
        <div class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden">
          ...
          <div class="p-5 text-center">
            <div id="liveClock" class="text-4xl font-extrabold text-[#8B0000] tracking-tight leading-none mb-1">00:00:00</div>
        ...
        <div class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden cal-card">
          <div class="px-5 py-4 border-b bg-gray-50 flex items-center justify-between">
            ...
            <span id="calYear" class="text-[10px] font-bold text-[#8B0000] bg-red-50 px-2 py-0.5 rounded-full">Academic Periods</span>
          </div>
          <div id="calendarList"
        ...
        <div class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden">
          ...
          <div class="p-4 space-y-2.5">
            <button onclick="openModal('addModal')" type="button" class="w-full flex items-center gap-3 bg-gradient-to-r from-red-50 to-white hover:from-red-100 hover:to-red-50 border border-red-100 rounded-lg px-4 py-3 text-left transition-all group">
...
<footer id="siteFooter" class="bg-[#8B0000] text-[#F4F4F4] p-6" style="margin-left:240px;">
  ...
  <div class="modal-box">
    <form method="POST" action="{{ route('admin.academic_periods.store') }}">
      ...
      <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white rounded-t-2xl z-10">
        ...
        <button type="button" onclick="closeModal('addModal')" class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-red-50 hover:text-[#8B0000] transition-all">
      ...
      <div class="p-6 space-y-4">
        <div>
          <label class="block text-[11px] font-bold text-gray-600 uppercase tracking-wide mb-1.5">Academic Year <span class="text-red-500">*</span></label>
          <div class="relative">
            <i class="fa-solid fa-graduation-cap absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
            <input name="academic_year" type="text" placeholder="e.g. 2026-2027" class="field-input w-full border border-gray-200 rounded-lg pl-9 pr-3 py-2.5 text-sm" required>
          </div>
        </div>

        <div>
          <label class="block text-[11px] font-bold text-gray-600 uppercase tracking-wide mb-2">Semester <span class="text-red-500">*</span></label>
          <div class="grid grid-cols-3 gap-2">
            <label class="cursor-pointer">
              <input type="radio" name="semester" value="1st Semester" class="sr-only peer" required>
              <div class="peer-checked:bg-[#8B0000] peer-checked:text-white peer-checked:border-[#8B0000] border-2 border-gray-200 rounded-xl p-3 text-center text-xs font-bold text-gray-500 hover:border-[#8B0000] hover:text-[#8B0000] transition-all select-none">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-[11px] font-bold text-gray-600 uppercase tracking-wide mb-1.5">Start Date <span class="text-red-500">*</span></label>
            <div class="relative">
              <i class="fa-solid fa-calendar-day absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
              <input name="start_date" type="date" class="field-input w-full border border-gray-200 rounded-lg pl-9 pr-3 py-2.5 text-sm" required>
            </div>
          </div>

          <div>
            <label class="block text-[11px] font-bold text-gray-600 uppercase tracking-wide mb-1.5">End Date <span class="text-red-500">*</span></label>
            <div class="relative">
              <i class="fa-solid fa-calendar-check absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
              <input name="end_date" type="date" class="field-input w-full border border-gray-200 rounded-lg pl-9 pr-3 py-2.5 text-sm" required>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea name="description" rows="2" class="field-input w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm resize-none"></textarea>
        </div>
        ...
        <div class="flex items-center justify-between rounded-xl border border-gray-200 bg-gray-50 px-4 py-3">
          ...
          <label class="relative inline-flex items-center cursor-pointer">
            ...
            <div class="w-10 h-5 bg-gray-200 rounded-full peer peer-checked:bg-[#8B0000] transition-colors after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-5"></div>
          </label>
        </div>
      </div>

      <div class="px-6 pb-6 pt-2 flex items-center justify-end gap-3">
        <button type="button" onclick="closeModal('addModal')" class="px-5 py-2.5 rounded-lg border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-all">Cancel</button>
        <button type="submit" class="px-6 py-2.5 rounded-lg bg-[#8B0000] hover:bg-[#760000] text-white text-sm font-bold shadow transition-all flex items-center gap-2">
  ...
  <div class="modal-box">
    <form method="POST" id="editForm">
      ...
      <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white rounded-t-2xl z-10">
        ...
        <button type="button" onclick="closeModal('editModal')" class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-red-50 hover:text-[#8B0000] transition-all">
          <i class="fa-solid fa-xmark"></i>
        </button>
      </div>
      ...
      <div class="p-6 space-y-4">
        <div>
          <label class="block text-[11px] font-bold text-gray-600 uppercase tracking-wide mb-1.5">Academic Year</label>
          <input type="text" name="academic_year" id="editYear" class="field-input w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm" required>
        </div>
        ...
        <div>
          ...
          <div class="grid grid-cols-3 gap-2">
            <label class="cursor-pointer">
              ...
              <div class="peer-checked:bg-[#8B0000] peer-checked:text-white peer-checked:border-[#8B0000] border-2 border-gray-200 rounded-xl p-3 text-center text-xs font-bold text-gray-500">1st Semester</div>
            </label>
            <label class="cursor-pointer">
              <input type="radio" name="semester" value="2nd Semester" class="sr-only peer edit-sem" required>
              <div class="peer-checked:bg-[#8B0000] peer-checked:text-white peer-checked:border-[#8B0000] border-2 border-gray-200 rounded-xl p-3 text-center text-xs font-bold text-gray-500">2nd Semester</div>
            </label>
            <label class="cursor-pointer">
              <input type="radio" name="semester" value="Summer" class="sr-only peer edit-sem" required>
              <div class="peer-checked:bg-[#8B0000] peer-checked:text-white peer-checked:border-[#8B0000] border-2 border-gray-200 rounded-xl p-3 text-center text-xs font-bold text-gray-500">Summer</div>
            </label>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="date" name="start_date" id="editStart" class="field-input w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm" required>
          </div>
          <div>
            <label class="block text-[11px] font-bold text-gray-600 uppercase tracking-wide mb-1.5">End Date</label>
            <input type="date" name="end_date" id="editEnd" class="field-input w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm" required>
          </div>
        </div>

        <div>
          <label class="block text-[11px] font-bold text-gray-600 uppercase tracking-wide mb-1.5">Description</label>
          <textarea rows="2" name="description" id="editDesc" class="field-input w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm resize-none"></textarea>
        </div>
      ...
      <div class="px-6 pb-6 pt-2 flex items-center justify-end gap-3">
        <button type="button" onclick="closeModal('editModal')" class="px-5 py-2.5 rounded-lg border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-all">Cancel</button>
        <button type="submit" class="px-6 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold shadow transition-all flex items-center gap-2">
    ...
    <form method="POST" id="deleteForm">
      ...
      <div class="p-6 text-center">
        ...
        <div class="flex gap-3">
          <button type="button" onclick="closeModal('deleteModal')" class="flex-1 py-2.5 rounded-lg border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-all">Cancel</button>
          <button type="submit" class="flex-1 py-2.5 rounded-lg bg-[#8B0000] hover:bg-[#760000] text-white text-sm font-bold shadow transition-all">
...
<script>
  ...
  let sidebarExpanded = true, openFlyout = null;
