...
<body class="bg-[#FAFAF8] text-[#1C1410]">
  ...
  <main id="mainContent" class="pt-[100px] px-4 sm:px-6 py-6 fade-up min-h-screen">
    <div class="mx-auto">
      ...
      <div class="mb-5 rounded-2xl overflow-hidden border border-[#EDE8E4] shadow-sm">
          ...
          <div class="bg-white px-5 py-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div>
                  ...
                  <p class="text-sm font-bold text-[#8B0000]">{{ $upcomingAppointment->service_type ?? '—' }}</p>
              </div>
              <div>
                  <p class="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-1">Date & Time</p>
              ...
              <div>
                  ...
                  <p class="text-sm font-bold text-[#1C1410]">{{ $upcomingAppointment->dentist_name ?? 'Dr. Nelson P. Angeles' }}</p>
              </div>
          </div>
      </div>
      @endif
      ...
      $latestDate = $totalRecords ? \Carbon\Carbon::parse($records->first()->appointment_date)->format('M Y') : null;
      @endphp
      ...
      <div class="records-body">
        ...
        <div class="flex items-center gap-3 mb-5 pt-2">
          <span style="font-size:9px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:#9E9690;">Visit
            History</span>
        ...
        <div class="space-y-0">
          ...
          $fmtRange = $apptTime->format('g:i A').' – '.$apptTime->copy()->addHour()->format('g:i A');
          @endphp
          <div class="rec-row" style="animation-delay:{{ $i * 0.06 }}s;">
            ...
            <div class="rec-card">
              ...
              <button type="button" class="rec-btn" onclick="openRecordModal(this)"
                ...
                data-time="{{ $fmtRange }}" data-status="completed" data-duration="{{ $record->duration ?? '—' }}"
                data-remarks="{{ $record->remarks ?? '' }}" data-oral="{{ $record->oral_examination ?? '' }}"
                data-diagnosis="{{ $record->diagnosis ?? '' }}" data-prescription="{{ $record->prescription ?? '' }}">
  ...
  <dialog id="record_modal">
    <div class="modal-inner">
      ...
      <div class="modal-body">
        ...
        <div class="chip-row">
          <div class="chip-box">
            <div class="chip-lbl"><i class="fa-solid fa-circle-check" style="color:#15803d;font-size:9px;"></i> Status
            </div>
            <span id="m_status" class="chip-val bg-emerald-100 text-emerald-800">—</span>
          </div>
          <div class="chip-box">
            <div class="chip-lbl"><i class="fa-regular fa-clock" style="font-size:9px;"></i> Duration</div>
  ...
  <script>
    ...
    function applyTheme(t) {
      ...
      topts.forEach(function (o) { o.classList.toggle("active", o.getAttribute("data-theme") === t); });
    ...
    topts.forEach(function (o) { o.addEventListener("click", function () { applyTheme(o.getAttribute("data-theme")); }); });

    /* ── SIDEBAR ── */
    var sidebarOpen = true;
    function applyLayout(w) { document.getElementById('sidebar').style.width = w; document.getElementById('mainContent').style.marginLeft = w; }
    function toggleSidebar() {
      var s = document.getElementById('sidebar'), tx = document.querySelectorAll('.sidebar-text');
      var ic = document.getElementById('sidebarIcon'), wr = document.getElementById('sidebarToggleWrapper');
      sidebarOpen = !sidebarOpen;
      if (sidebarOpen) { applyLayout('220px'); s.classList.replace('collapsed', 'expanded'); tx.forEach(function (t) { t.classList.remove('opacity-0', 'w-0'); t.classList.add('opacity-100'); }); wr.classList.replace('justify-center', 'justify-end'); ic.classList.replace('fa-bars', 'fa-xmark'); }
      else { applyLayout('72px'); s.classList.replace('expanded', 'collapsed'); tx.forEach(function (t) { t.classList.add('opacity-0', 'w-0'); t.classList.remove('opacity-100'); }); wr.classList.replace('justify-end', 'justify-center'); ic.classList.replace('fa-xmark', 'fa-bars'); }
      applyTheme(localStorage.getItem("theme") || "light");
    }
    ...
    function closeRecModal() { recModal.close(); }

    /* ── DOM READY ── */
    document.addEventListener('DOMContentLoaded', function () {
      if (window.innerWidth >= 768) { sidebarOpen = true; applyLayout('220px'); }
      else { document.getElementById('mainContent').style.marginLeft = '0'; }
      ...
      recModal.addEventListener('click', function (e) {
        ...
        if (e.clientX < r.left || e.clientX > r.right || e.clientY < r.top || e.clientY > r.bottom) closeRecModal();
      });

      /* Mobile FAB */
      var mf = document.getElementById('mobFab'), mm = document.getElementById('mobFabMenu');
      if (mf && mm) {
        mf.addEventListener('click', function (e) { e.stopPropagation(); var o = mm.classList.contains('open'); mm.classList.toggle('open', !o); mf.classList.toggle('open', !o); });
        mm.addEventListener('click', function (e) { e.stopPropagation(); });
      }

      /* Notifications */
      var nb = document.getElementById("notifBtn"), nm = document.getElementById("notifMenu");
      if (nb && nm) {
        ...
        document.addEventListener("keydown", function (e) { if (e.key === "Escape") nm.classList.remove("open"); });
      }

      /* Outside clicks */
      document.addEventListener('click', function (e) {
        if (mm) { mm.classList.remove('open'); if (mf) mf.classList.remove('open'); }
