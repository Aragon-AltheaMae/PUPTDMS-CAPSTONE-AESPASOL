...
<main id="mainContent" class="patient-page-shell page-enter patient-signature-review-page">
    <div class="mx-auto w-full max-w-[1480px] px-4 md:px-5 xl:px-6">
        <div class="relative overflow-hidden rounded-[32px] border border-[#f0dfd7] bg-[radial-gradient(circle_at_top_left,_rgba(139,0,0,0.08),_transparent_35%),linear-gradient(180deg,_#fffdfc_0%,_#fff8f5_100%)] shadow-[0_24px_60px_rgba(90,40,24,0.08)]">
            ...
            <div class="grid gap-6 px-5 py-5 md:px-7 md:py-6 xl:grid-cols-[0.9fr_1.1fr]">
                <section class="space-y-5">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                            <p class="text-xs font-bold uppercase tracking-[0.34em] text-[#8B0000]/75">Patient Signature</p>
                            <h1 class="mt-3 whitespace-nowrap text-[2rem] font-extrabold leading-[1.03] text-[#2b1f1f] md:text-[2.3rem]">
                                Re-submit your clinic signature
                            </h1>
                            <p class="mt-3 max-w-[560px] text-[13.5px] leading-6 text-[#5c5550]">
                                Your last signature was flagged during manual review. Please upload a clearer signature image
                    ...
                    <div class="rounded-[28px] border border-[#f1d7d0] bg-white/90 p-4 shadow-sm md:p-5">
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div class="inline-flex items-center gap-2 rounded-full bg-[#fff1ef] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.14em] text-[#a31616]">
                                <i class="fa-solid fa-circle-exclamation text-[10px]"></i>
                                Re-upload Required
                            </div>

                            @if ($medicalHistory->patient_signature)
                            <a href="{{ asset('storage/' . $medicalHistory->patient_signature) }}" target="_blank" rel="noopener noreferrer"
                        ...
                        <div class="mt-4 grid gap-4 lg:grid-cols-[0.88fr_1.12fr]">
                            <div class="space-y-4">
                                <div class="rounded-2xl border border-[#f4d5cf] bg-[#fff7f5] p-4">
                                    <p class="text-xs font-extrabold uppercase tracking-[0.16em] text-[#8B0000]">Review Note</p>
                                ...
                                <div class="rounded-2xl border border-[#efe5de] bg-[#fffdfa] p-4">
                                    <p class="text-xs font-extrabold uppercase tracking-[0.16em] text-[#8B0000]">What To Submit</p>
                                    <div class="mt-3 space-y-2 text-[13px] leading-5 text-[#5c5550]">
                                        <p><i class="fa-solid fa-check mr-2 text-[#8B0000]"></i>Your actual handwritten signature only</p>
                                        <p><i class="fa-solid fa-check mr-2 text-[#8B0000]"></i>Clear background and readable strokes</p>
                                        <p><i class="fa-solid fa-xmark mr-2 text-[#b45309]"></i>No selfies, posters, screenshots, or documents</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="rounded-2xl border border-[#efe5de] bg-[#fffdfa] p-4">
                                <div>
                                    <p class="text-xs font-extrabold uppercase tracking-[0.16em] text-[#8B0000]">Previous Uploaded Image</p>
                                    <p class="mt-1 text-sm text-[#7a746d]">This is the image that was marked invalid.</p>
                                </div>

                                <div class="mt-4 overflow-hidden rounded-[24px] border border-[#eadfd8] bg-[#f8f5f2]">
                                    <img src="{{ asset('storage/' . $medicalHistory->patient_signature) }}"
                                        alt="Previous uploaded signature" class="h-full max-h-[360px] w-full object-contain">
                                </div>
                            </div>
                            @endif
                        </div>
                ...
                <section class="rounded-[30px] border border-[#eadfd8] bg-white p-5 shadow-[0_20px_50px_rgba(83,34,18,0.08)] md:p-6">
                    <div class="mb-4">
                        <p class="text-xs font-extrabold uppercase tracking-[0.16em] text-[#8B0000]">Submit A New Signature</p>
                        ...
                        <p class="mt-2 text-[13px] leading-6 text-[#6b6470]">
                            You can either upload a signature image or draw directly here using your mouse, touch, or stylus.
                        </p>
                    </div>

                    <form method="POST" action="{{ route('patient.signature-review.update') }}" enctype="multipart/form-data"
                        class="space-y-4" id="signatureReviewForm">
                        @csrf

                        <div class="space-y-4">
                            <div class="inline-flex w-fit max-w-full rounded-[22px] border border-[#e8e2dd] bg-[#faf7f4] p-1 shadow-sm">
                            ...
                            <div id="uploadPanel" class="signature-mode-panel rounded-[28px] border border-dashed border-[#e3d7d0] bg-[#fffdfc] p-5">
                                <div class="flex min-h-[270px] flex-col items-center justify-center text-center">
                                    <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#fff2ef] text-[#8B0000] shadow-sm">
                            ...
                            <div id="drawPanel" class="signature-mode-panel hidden rounded-[28px] border border-[#efe4dc] bg-[#fffdfb] p-5">
                                <p class="text-center text-[13px] font-extrabold text-[#8B0000]">Or draw your signature here</p>

                                <div class="mt-4 overflow-hidden rounded-3xl border-2 border-dashed border-[#ddd4cf] bg-white p-3">
                                    <canvas id="signatureCanvas" class="block h-[190px] w-full touch-none rounded-2xl bg-white"></canvas>
                                </div>
                        ...
                        <div id="signature_result_box" class="hidden rounded-2xl border border-[#e8e2dd] bg-[#fffdfa] p-4">
                            <p id="signature_filename" class="truncate text-xs font-bold text-[#5c5550]"></p>
                            <div id="signature_error" class="mt-2 hidden rounded-2xl px-3 py-3 text-xs font-semibold leading-5"></div>
                        </div>
                        ...
                        <div class="rounded-3xl border border-[#f3e5b8] bg-[#fff7df] p-4 text-[13px] text-[#8a5310]">
                            ...
                            <p class="mt-1 leading-6">
                                If you change your drawing after clicking `Use Drawn Signature`, click it again so the new version becomes the one that gets submitted.
                            </p>
                        </div>
...
<script>
    ...
    function showSignatureStatus(fileName = "", message = "", type = "neutral") {
        ...
        let className = "mt-2 rounded-2xl border border-[#e8e2dd] bg-white px-3 py-3 text-xs text-[#5c5550] leading-5 font-semibold";

        if (type === "success") {
            icon = `<i class="fa-solid fa-circle-check mr-1"></i>`;
            className = "mt-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-3 py-3 text-xs text-emerald-700 leading-5 font-semibold";
        } else if (type === "error") {
            icon = `<i class="fa-solid fa-circle-exclamation mr-1"></i>`;
            className = "mt-2 rounded-2xl border border-red-200 bg-red-50 px-3 py-3 text-xs text-red-700 leading-5 font-semibold";
        }
    ...
    function showSignatureError(fileName = "", result = {}) {
        ...
        if (sigError) {
            sigError.className = "mt-2 rounded-2xl border border-red-200 bg-red-50 px-3 py-3 text-xs text-red-700 leading-5 font-semibold";
            sigError.innerHTML = `
    ...
    function invalidateDrawnSignatureAfterEdit() {
        ...
        showSignatureStatus("", "Draw changed. Click Use Drawn Signature again to submit the latest version.", "neutral");
    }
    ...
    document.getElementById("signatureReviewForm")?.addEventListener("submit", (event) => {
        if (signatureAiChecking) {
            event.preventDefault();
            showSignatureStatus(sigInput?.files?.[0]?.name || "", "Please wait while the signature is being checked.", "neutral");
            return;
        }
