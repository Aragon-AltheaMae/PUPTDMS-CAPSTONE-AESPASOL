...
@section('content')
    ...
    <main id="mainContent" class="pt-[90px] md:pt-[104px] px-4 sm:px-6 md:px-8 pb-6 fade-up min-h-screen bg-[#F4F4F4]">
                    ...
                    <div id="calendarSkeletonContainer"
                        ...
                        <div class="animate-pulse space-y-6">
                            ...
                            <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:10px;">
                                @for ($i = 0; $i < 35; $i++)
                                    <div class="h-10 sm:h-12 bg-gray-100 rounded-xl">
                                    </div>
                                @endfor
                            </div>
                        </div>
            ...
            <dialog id="dentalClearanceModal" class="modal">
                <form id="clearanceRequestForm" method="POST" action="{{ route('patient.document.requests.store') }}"
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            <label
                                class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Purpose</label>
    ...
    <script>
        ...
        function renderHero() {
            ...
            if (h >= 6 && h < 12) {
                ...
                greetingIcon = 'fa-solid fa-sun text-yellow-400 text-sm greet-spin';
            } else if (h >= 12 && h < 18) {
        ...
        function renderUpcomingAppointment() {
            ...
            if (d.exists) {
                ...
                wrapper.innerHTML =
                    ...
                    '<span class="inline-flex w-max items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold border ' +
                    statusPillCls + '">' +
                    ...
                    '<div class="bg-gray-50/50 p-4 rounded-xl border border-gray-100"><p class="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1.5">Service</p><p class="text-sm sm:text-base font-bold text-[#8B0000]">' +
                    escapeHtml(d.service) + '</p></div>' +
                    '<div class="bg-gray-50/50 p-4 rounded-xl border border-gray-100"><p class="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1.5">Date &amp; Time</p><p class="text-sm sm:text-base font-bold text-gray-800">' +
                    escapeHtml(d.date) + '<span class="text-gray-300 mx-2">|</span>' + escapeHtml(d.time_fmt) +
                    '</p></div>' +
                    '<div class="bg-gray-50/50 p-4 rounded-xl border border-gray-100"><p class="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1.5">Dentist</p><p class="text-sm sm:text-base font-bold text-gray-800">' +
                    escapeHtml(d.dentist) + '</p></div>' +
                    '</div>' +
                    '<div class="mt-5 pt-4 border-t border-gray-100 flex justify-end">' +
                    '<a href="' + escapeHtml(d.indexUrl) +
                    '" class="inline-flex items-center gap-2 text-sm font-bold text-[#8B0000] hover:text-[#5A0000] transition-colors bg-red-50 px-4 py-2 rounded-lg">Manage Appointment <i class="fa-solid fa-arrow-right text-xs"></i></a>' +
                    '</div></div></div>';
            } else {
                wrapper.innerHTML =
                    '<div class="bg-white rounded-[1.25rem] border border-gray-200 shadow-sm overflow-hidden fade-up">' +
        ...
        function renderProfile() {
            ...
            document.getElementById("profileSkeletonContainer").innerHTML =
                ...
                '</div></div>' +
                '<div class="flex-1"><h3 class="font-extrabold text-xl sm:text-2xl leading-tight mb-1">' + escapeHtml(pData
                    .name) + '</h3>' +
                '<p class="text-sm font-medium text-white/70 mb-3">Student</p>' +
                '<span class="inline-block bg-black/20 text-yellow-300 border border-yellow-300/30 text-xs font-bold px-3 py-1 rounded-lg tracking-wider">' +
                escapeHtml(pData.patientId) + '</span>' +
                '</div></div>' +
                ...
                pData.rows.map(function(row) {
                    return '<div class="flex flex-col sm:flex-row sm:items-center px-6 py-3.5 gap-1 sm:gap-4 hover:bg-gray-50 transition-colors">' +
                        '<span class="text-gray-500 uppercase text-[10px] font-bold tracking-widest sm:w-32 flex-shrink-0">' +
                        escapeHtml(row[0]) + '</span>' +
            ...
            function renderCalendar(year, month) {
                ...
                for (var d = 1; d <= totalDays; d++) {
                    ...
                    if (isToday) {
                        ...
                        textClass = 'font-semibold text-gray-600';
                    }
                    if (myAppt) {
                    ...
                    var tooltip = tooltipTxt ?
                        '<div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 z-50 bg-gray-900 text-white text-[11px] font-bold px-3 py-1.5 rounded-lg whitespace-nowrap shadow-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200">' +
                        tooltipTxt +
                        '<div class="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-gray-900"></div></div>' :
                        '';
                    cells += '<div class="relative group flex items-center justify-center py-1">' + tooltip +
                        '<div class="relative w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center text-sm rounded-full transition-all duration-200 ' +
                        bgClass + ' ' + textClass + ' ' + ringClass + ' cursor-default">' + d + dotHtml + '</div></div>';
                }

                var headerHtml = dayLabels.map(function(l, i) {
                    return '<div class="text-center text-[10px] sm:text-xs font-bold ' + (i === 0 || i === 6 ?
                        'text-gray-400' : 'text-gray-500') + ' uppercase tracking-wider py-2">' + l + '</div>';
                }).join('');
                ...
                document.getElementById("calendarSkeletonContainer").innerHTML =
                    ...
                    '<span class="w-24 text-center text-sm font-bold text-[#8B0000]">' + monthNames[month] + ' ' + year +
                    '</span>' +
                    ...
                    '<div class="flex items-center gap-1.5 text-[10px] sm:text-xs text-gray-500 font-semibold"><span class="w-2.5 h-2.5 rounded-full bg-green-500 inline-block"></span>My Appointment</div>' +
                    '<div class="flex items-center gap-1.5 text-[10px] sm:text-xs text-gray-500 font-semibold"><span class="w-2.5 h-2.5 rounded-full bg-[#8B0000] inline-block"></span>Today</div>' +
                    '<div class="flex items-center gap-1.5 text-[10px] sm:text-xs text-gray-500 font-semibold"><span class="w-2.5 h-2.5 rounded-full bg-red-500 inline-block"></span>Full Schedule</div>' +
                    '<div class="flex items-center gap-1.5 text-[10px] sm:text-xs text-gray-500 font-semibold"><span class="w-2.5 h-2.5 rounded-full bg-blue-400 inline-block"></span>Holiday</div>' +
                    '</div></div>';
            }
