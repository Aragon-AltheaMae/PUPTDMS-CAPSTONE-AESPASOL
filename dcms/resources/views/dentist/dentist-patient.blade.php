    ...
    <div class="px-6 py-4 flex flex-col gap-6 flex-1 overflow-y-auto scroll-smooth bg-white">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          ...
          <div class="grid grid-cols-2 gap-y-3">
            <label class="flex items-center gap-2 cursor-pointer hover:opacity-80 transition"><input type="radio"
                name="year" value="1st Year" class="filter-input radio-red" /> <span
                class="text-sm font-semibold text-gray-700">1st Year</span></label>
            <label class="flex items-center gap-2 cursor-pointer hover:opacity-80 transition"><input type="radio"
                name="year" value="3rd Year" class="filter-input radio-red" /> <span
                class="text-sm font-semibold text-gray-700">3rd Year</span></label>
            <label class="flex items-center gap-2 cursor-pointer hover:opacity-80 transition"><input type="radio"
                name="year" value="2nd Year" class="filter-input radio-red" /> <span
                class="text-sm font-semibold text-gray-700">2nd Year</span></label>
            <label class="flex items-center gap-2 cursor-pointer hover:opacity-80 transition"><input type="radio"
                name="year" value="4th Year" class="filter-input radio-red" /> <span
                class="text-sm font-semibold text-gray-700">4th Year</span></label>
          </div>
        </div>
        <div>
      ...
      <div class="pb-6">
        ...
        <div class="flex flex-wrap gap-x-6 gap-y-3">
          ...
          <label class="flex items-center gap-2 cursor-pointer hover:opacity-80 transition"><input type="radio"
              name="department" value="Dependent" class="filter-input radio-red" /> <span
              class="text-sm font-semibold text-gray-700">Dependent</span></label>
        </div>
      </div>
    </div>
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    try {
      ...
      function clearFormState() {
        ...
        selectedDepartment = null; selectedProgram = null;
        selectedYearLevel = null; selectedSection = null;
        activeFromDate = ""; activeToDate = "";

        document.querySelectorAll('#fSortGroup .ftag').forEach(function (b) { b.classList.remove('ftag-active'); });
      ...
      function renderFilterChips() {
        ...
        if (fDate && tDate && (fDate.value || tDate.value)) {
          var lbl = fDate.value && tDate.value ? (fDate.value + " to " + tDate.value) : (fDate.value ? "From " + fDate.value : "Until " + tDate.value);
          addChip(lbl, function () { fDate.value = ""; tDate.value = ""; });
        }

        var course = document.querySelector('input[name="course"]:checked');
        if (course) addChip(course.value, function () { course.checked = false; });

        var year = document.querySelector('input[name="year"]:checked');
        if (year) addChip(year.value, function () { year.checked = false; });

        var sectionElem = document.querySelector('input[name="section"]:checked');
        if (sectionElem) addChip("Section " + sectionElem.value, function () { sectionElem.checked = false; });

        var dept = document.querySelector('input[name="department"]:checked');
        if (dept) addChip(dept.value, function () { dept.checked = false; });
      ...
      function getInfo(p) {
        ...
        var parts = ((infoEl ? infoEl.textContent.trim() : "") || "").split("|").map(function (s) { return s.trim(); });
        return { program: parts[0] || "", year: parts[1] || "", section: parts[2] || "", dateStr: parts[3] || "", department: parts[4] || p.getAttribute("data-department") || "" };
      }
      function getName(p) { var el = p.querySelector(".font-semibold"); return (el ? el.textContent : "").trim(); }
      function getService(p) { var el = p.querySelector(".truncate"); return (el ? el.textContent : "").trim(); }
      ...
      if (applyFiltersBtn) {
        applyFiltersBtn.onclick = function () {
          ...
          if (sortVal === 'az') { nameSort = 'az'; dateSort = null; }
          else if (sortVal === 'za') { nameSort = 'za'; dateSort = null; }
          else if (sortVal === 'newest') { dateSort = 'desc'; nameSort = null; }
          else if (sortVal === 'oldest') { dateSort = 'asc'; nameSort = null; }
      ...
      function renderPagination(items) {
        ...
        for (var i = 1; i <= total; i++) {
          (function (pageNum) {
            ...
            btn.onclick = function () { currentPage = pageNum; updatePage(); };
      ...
      function updatePage() {
        ...
        if (currentItems.length === 0) {
          ...
          if (isSearching) {
            ...
            extraHtml = '<button id="clearFiltersInline" class="mt-3 px-4 py-2 rounded-xl border border-dashed border-gray-300 text-sm text-gray-400 hover:border-[#8B0000] hover:text-[#8B0000] hover:bg-[#8B0000]/5 transition-all duration-200"><i class="fa-solid fa-xmark mr-1.5 text-xs"></i> Reset</button>';
          } else {
            var msg = emptyMessages[activeTab] || emptyMessages.all;
            icon = msg.icon; title = msg.title; sub = msg.sub;
          }

          patientContainer.innerHTML = '<div class="flex flex-col items-center justify-center py-16 text-center gap-2"><div class="w-16 h-16 rounded-2xl bg-gray-50 flex items-center justify-center mb-2"><i class="fa-solid ' + icon + ' text-2xl text-gray-300"></i></div><p class="text-base font-bold text-gray-500">' + title + '</p><p class="text-sm text-gray-400 max-w-xs">' + sub + '</p>' + extraHtml + '</div>';
      ...
      if (prevPageBtn) { prevPageBtn.onclick = function () { if (currentPage > 1) { currentPage--; updatePage(); } }; }
      if (nextPageBtn) { nextPageBtn.onclick = function () { if (currentPage < Math.ceil(currentItems.length / PER_PAGE)) { currentPage++; updatePage(); } }; }

      function applyFilters() {
        var data = allPatients.slice();

        if (searchKeyword) {
          data = data.filter(function (p) { return matchesSearch(p, searchKeyword); });
        } else {
          ...
          if (activeFromDate || activeToDate) data = data.filter(function (p) {
            ...
            if (isNaN(d.getTime())) return false; // Iwas error sa invalid date
