...
<body class="bg-[#F4F4F4] text-[#333333] font-normal min-h-screen flex flex-col">
  ...
  <!-- SIDEBAR -->
  <aside id="sidebar"
    class="fixed left-0 top-[80px] h-[calc(100vh-80px)] w-[72px] bg-[#FAFAFA] drop-shadow-xl transition-all duration-300 flex flex-col justify-between z-40">
    ...
    <div>
      <div id="sidebarToggleWrapper" class="flex items-center justify-center px-4 py-6 transition-all duration-300">
        <button onclick="toggleSidebar()" id="sidebarToggleBtn"
          class="w-10 h-10 flex items-center justify-center rounded-full text-[#757575] hover:text-[#8B0000] hover:bg-[#D9D9D9] transition-all duration-300">
      ...
      <nav class="space-y-2 px-3 text-gray-600 text-sm">
        ...
        <a href="{{ route('dentist.dashboard') }}"
          class="sidebar-link relative rounded-xl transition-all duration-200 hover:bg-[#8B0000] hover:text-[#F4F4F4]
    ...
    <div class="px-3 pb-5 space-y-2">
      <button id="themeToggle"
        class="sidebar-link relative flex items-center justify-center w-full px-2 py-1.5 rounded-xl bg-[#7B6CF6] text-[#F4F4F4] transition-all duration-200 hover:scale-105"
  ...
  <!-- ================= MAIN (matches Patient List pattern) ================= -->
 <main
  id="mainContent"
  class="pt-[100px] px-6 py-10 w-full flex-1 transition-transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]">
    <!-- CONTENT WRAPPER -->
    <div class="max-w-7xl mt-4 mx-auto fade-in">
      ...
      <div class="flex justify-center mt-14">
        <div class="w-[760px] h-14 bg-[#7a0000] rounded-full p-1.5 flex shadow-md">
          <button id="btnUpcoming" class="flex-1 rounded-full bg-[#8b0000] text-white text-sm font-semibold transition-all duration-200">
            Upcoming Appointments
          </button>
          <button id="btnPast" class="flex-1 rounded-full text-white/80 text-sm font-semibold transition-all duration-200">
            Past Appointments
          </button>
        </div>
      </div>
      ...
      <section id="upcomingSection" class="mt-14 flex justify-center">
        <div class="w-[1100px]">
          <div class="relative pl-10">
            ...
            <div class="grid grid-cols-[36px_1.8fr_1.2fr_1.8fr_1.8fr_1.2fr_1.6fr] text-[13px] font-semibold text-[#8b0000] pb-3 border-b border-gray-400 mb-6 px-8">
              <div></div>
              <p>Date</p>
              <p>Time</p>
              <p>Service</p>
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-200 px-8 py-5">
              <div class="grid grid-cols-[36px_1.8fr_1.2fr_1.8fr_1.8fr_1.2fr_1.6fr] items-center text-[13px] text-[#8b0000]">
                ...
                <div class="flex flex-col gap-2 items-end pr-2">
                  <button onclick="openStartProcedureModal()" class="h-6 w-[130px] rounded-md text-[11px] font-semibold text-white bg-green-600 hover:bg-green-700 transition">
                    Start Procedure
                  </button>
                  <button onclick="openRescheduleModal()" class="h-6 w-[130px] rounded-md text-[11px] font-semibold text-black bg-yellow-400 hover:bg-yellow-500 transition">
                    Reschedule
                  </button>
                  <button onclick="openCancelAppointmentModal()" class="h-6 w-[130px] rounded-md text-[11px] font-semibold text-white bg-[#8b0000] hover:bg-[#6f0000] transition">
                    Cancel
                  </button>
                </div>
              </div>
      ...
      <section id="pastSection" class="mt-14 flex justify-center hidden">
        <div class="w-[1100px]">
          <div class="relative pl-10">
            ...
            <div class="grid grid-cols-[32px_1.8fr_1.2fr_1.8fr_1.8fr_1.2fr] text-[13px] font-semibold text-[#8b0000] pb-3 border-b border-gray-400 mb-6 px-8">
              <div></div>
              <p>Date</p>
              <p>Time</p>
              <p>Service</p>
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-200 px-8 py-5">
              <div class="grid grid-cols-[32px_1.8fr_1.2fr_1.8fr_1.8fr_1.2fr] items-center text-[13px] text-[#8b0000]">
                <div class="flex justify-center">
                  <i class="fa-solid fa-ellipsis-vertical"></i>
                </div>
  ...
  <!-- ✅ FOOTER OUTSIDE MAIN (same pattern as Patient List) -->
  ...
  <!-- RESCHEDULE MODAL -->
  <div id="rescheduleModal" class="fixed inset-0 bg-black/40 flex items-center justify-center hidden z-50">
    <div class="bg-white w-[520px] rounded-2xl p-10 text-center shadow-xl">
      <h2 class="text-2xl font-bold text-[#8b0000] mb-6">Reschedule Appointment</h2>
      <p class="text-lg font-semibold text-[#8b0000] mb-2">Are you sure you want to<br>reschedule this appointment?</p>
      <p class="text-sm text-gray-500 mb-10">You will be able to select a new date and time.</p>

      <div class="flex justify-center gap-6">
        <button onclick="closeRescheduleModal()" class="px-8 py-2 rounded-full border border-[#8b0000] text-[#8b0000] font-semibold hover:bg-[#8b0000] hover:text-white transition">
          Cancel
        </button>
        <button onclick="confirmReschedule()" class="px-10 py-2 rounded-full bg-[#8b0000] text-white font-semibold hover:bg-[#6f0000] transition">
          Yes
        </button>
      </div>
    </div>
  ...
  <div id="startProcedureModal" class="fixed inset-0 bg-black/40 flex items-center justify-center hidden z-50">
    <div class="bg-white w-[620px] min-h-[320px] rounded-2xl p-10 shadow-xl">
      <h2 class="text-2xl font-semibold text-black mb-8">Confirm the start of procedure?</h2>

      <div class="flex items-center gap-4 mb-10">
        <label class="text-[#8b0000] font-medium w-[80px]">Patient:</label>
        <input type="text" id="patientNameInput" value="Alilah Gomez" readonly
          class="h-10 w-[360px] rounded-md px-4 bg-[#8b0000] text-white font-medium cursor-not-allowed focus:outline-none" />
      </div>

      <div class="flex gap-4">
        <button onclick="confirmStartProcedure()" class="bg-green-500 hover:bg-green-600 text-white px-10 py-2 rounded-md font-semibold transition">
          START
        </button>
        <button onclick="closeStartProcedureModal()" class="bg-gray-400 hover:bg-gray-500 text-white px-10 py-2 rounded-md font-semibold transition">
          BACK
        </button>
      </div>
    </div>
  ...
  <div id="cancelAppointmentModal" class="fixed inset-0 bg-black/40 flex items-center justify-center hidden z-50">
    <div class="bg-white w-[520px] rounded-2xl p-10 text-center shadow-xl">
      <h2 class="text-2xl font-bold text-black mb-6">Cancel Appointment</h2>
      <p class="text-lg font-semibold text-[#8b0000] mb-2">Are you sure you want to<br>cancel this appointment?</p>
      <p class="text-sm text-gray-500 mb-10">This action cannot be undone.</p>

      <div class="flex justify-center gap-6">
        <button onclick="closeCancelAppointmentModal()" class="px-8 py-2 rounded-full border border-[#8b0000] text-[#8b0000] font-semibold hover:bg-[#8b0000] hover:text-white transition">
          No
        </button>
        <button onclick="confirmCancelAppointment()" class="px-10 py-2 rounded-full bg-[#8b0000] text-white font-semibold hover:bg-[#6f0000] transition">
          Yes
        </button>
      </div>
    </div>
