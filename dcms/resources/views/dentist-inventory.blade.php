...
<body class="bg-[#F4F4F4] min-h-screen flex flex-col">
    ...
    <div class="pt-4">
      ...
      <nav class="space-y-2 px-3 text-gray-600">
        ...
        <a href="{{ route('dentist.documentrequests') }}"
          ...
          <span class="w-8 h-8 rounded-lg flex items-center justify-content:center transition-colors duration-200 ml-1">
  ...
  <main id="mainContent" class="pt-[88px] px-6 pb-6 min-h-screen fade-in">
    <div class="max-w-7xl mt-4 mx-auto">
      ...
      <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-[#660000]">Inventory</h1>
  ...
  <script>
    ...
    /**
     * Format stock number as NN-NNN (digits only, dash auto-inserted at position 2).
     * User just types digits — the dash is always present and cannot be removed.
     */
    function formatStockNo(input) {
      // Preserve cursor position awareness — strip all non-digits
      let digits = input.value.replace(/\D/g, '');

      // Clamp to 5 digits max (NN + NNN)
      if (digits.length > 5) digits = digits.slice(0, 5);

      // Rebuild with the fixed dash separator
      if (digits.length <= 2) {
        input.value = digits;
      } else {
    ...
    /** Apply / clear validation state on a single field */
    function setFieldState(id, errorMsg) {
      ...
      if (errorMsg) {
        ...
        if (errEl) errEl.innerHTML =
          `<i class="fa-solid fa-circle-exclamation" style="font-size:9px;"></i> ${errorMsg}`;
      } else {
    ...
    function validateAddField(id) {
      ...
      switch (id) {
        ...
        case 'addStock':
          ...
          if (!/^\d{2}-\d{3}$/.test(val)) {
            setFieldState(id, 'Must be in format 00-000 (digits only)');
            return false;
          }
          break;
    ...
    /** Validate all add-form fields; returns true only if all pass */
    function validateAllAddFields() {
      const fields = ['addCategory', 'addDate', 'addStock', 'addUnit', 'addName', 'addQty', 'addUsed'];
      return fields.map(id => validateAddField(id)).every(Boolean);
    }
