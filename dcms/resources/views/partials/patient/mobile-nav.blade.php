<nav id="mobileBottomNav" data-dashboard-url="{{ route('homepage') }}"
    class="fixed bottom-6 left-1/2 -translate-x-1/2 bg-white/95 backdrop-blur-md shadow-[0_12px_40px_rgba(0,0,0,0.15)] z-40 rounded-full border border-gray-100 transition-colors duration-300 dark:bg-[#0d1117]/95 dark:border-[#21262d] dark:shadow-[0_12px_40px_rgba(0,0,0,0.6)]">
    ...
    <div class="grid grid-cols-5 items-center w-full h-[66px] relative z-10 px-1">
        ...
        <a href="{{ route('homepage') }}"
            ...
            <i
                class="fa-solid fa-house text-[18px] mb-1 transition-transform duration-300 {{ $isHome ? 'scale-110' : '' }}"></i>
            <span class="text-[10px] font-bold">Home</span>
            <span
                class="absolute bottom-1 w-1 h-1 rounded-full bg-[#8B0000] dark:bg-[#ff6b6b] transition-opacity duration-300 {{ $isHome ? 'opacity-100' : 'opacity-0' }}"></span>
        </a>
        ...
        <a href="{{ route('patient.appointment.index') }}"
            ...
            <i
                class="fa-solid fa-calendar-check text-[18px] mb-1 transition-transform duration-300 {{ $isAppt ? 'scale-110' : '' }}"></i>
            <span class="text-[10px] font-bold">Appts</span>
            <span
                class="absolute bottom-1 w-1 h-1 rounded-full bg-[#8B0000] dark:bg-[#ff6b6b] transition-opacity duration-300 {{ $isAppt ? 'opacity-100' : 'opacity-0' }}"></span>
        </a>

        <div class="relative w-full h-full flex items-center justify-center">

            <div id="mobFabMenu"
                class="fixed left-1/2 bottom-[96px] w-[230px] -translate-x-1/2 bg-white dark:bg-[#161b22] border border-gray-100 dark:border-[#21262d] rounded-2xl shadow-[0_12px_40px_rgba(0,0,0,0.15)] py-2 z-[70] overflow-hidden origin-bottom">
                ...
                <a href="{{ route('patient.book.appointment') }}"
                    ...
                    <div
                        class="w-8 h-8 rounded-full bg-red-50 dark:bg-red-900/30 text-[#8B0000] dark:text-red-400 flex items-center justify-center flex-shrink-0">
        ...
        <a href="{{ route('patient.record') }}"
            ...
            <i
                class="fa-solid fa-folder-open text-[18px] mb-1 transition-transform duration-300 {{ $isRec ? 'scale-110' : '' }}"></i>
            <span class="text-[10px] font-bold">Records</span>
            <span
                class="absolute bottom-1 w-1 h-1 rounded-full bg-[#8B0000] dark:bg-[#ff6b6b] transition-opacity duration-300 {{ $isRec ? 'opacity-100' : 'opacity-0' }}"></span>
        </a>
        ...
        <a href="{{ route('patient.about.us') }}"
            ...
            <i
                class="fa-solid fa-circle-info text-[18px] mb-1 transition-transform duration-300 {{ $isAbout ? 'scale-110' : '' }}"></i>
            <span class="text-[10px] font-bold">About</span>
            <span
                class="absolute bottom-1 w-1 h-1 rounded-full bg-[#8B0000] dark:bg-[#ff6b6b] transition-opacity duration-300 {{ $isAbout ? 'opacity-100' : 'opacity-0' }}"></span>
        </a>
